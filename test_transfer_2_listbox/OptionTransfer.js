...
function OT_update()
{
		...
		{
			var o=this.left.options[i];newLeft[o.value]=1;
		...
		{
			var o=this.right.options[i];newRight[o.value]=1;
		...
		{
			this.removedLeftField.value = this.prefix + OT_join(removedLeft,this.delimiter) + this.postfix;
		}
		if(this.removedRightField!=null)
		{
			this.removedRightField.value = this.prefix + OT_join(removedRight,this.delimiter)+ this.postfix;
		}
		if(this.addedLeftField!=null)
		{
			this.addedLeftField.value = this.prefix +  OT_join(addedLeft,this.delimiter)+ this.postfix;
		}
		if(this.addedRightField!=null)
		{
			this.addedRightField.value = this.prefix + OT_join(addedRight,this.delimiter)+ this.postfix;
		}
		if(this.newLeftField!=null)
		{
			this.newLeftField.value = this.prefix + OT_join(newLeft,this.delimiter)+ this.postfix;
		}
		if(this.newRightField!=null)
		{
			this.newRightField.value = this.prefix + OT_join(newRight,this.delimiter)+ this.postfix;
		}
}
...
function OT_setPrefix(val){this.prefix=val;}
function OT_setPostfix(val){this.postfix=val;}
...
function OptionTransfer(l,r)
{
	...
	this.setPrefix=OT_setPrefix;	
	this.setPostfix=OT_setPostfix;	
