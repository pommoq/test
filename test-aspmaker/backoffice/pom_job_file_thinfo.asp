...
' ASPMaker configuration for Table job_file_th
Dim job_file_th
...
Class cjob_file_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "job_file_id", job_file_id)
		Call ew_SetArObj(Fields, "job_id", job_id)
		Call ew_SetArObj(Fields, "job_file_name", job_file_name)
		Call ew_SetArObj(Fields, "job_file_title", job_file_title)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "job_file_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [job_file_th]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[job_file_id] = @job_file_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(job_file_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@job_file_id@", ew_AdjustSql(job_file_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_job_file_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_job_file_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_job_file_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_job_file_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_job_file_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_job_file_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_job_file_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(job_file_id.CurrentValue) Then
			sUrl = sUrl & "job_file_id=" & job_file_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("job_file_id") ' job_file_id
	...
	Public Sub LoadListRowValues(RsRow)
		job_file_id.DbValue = RsRow("job_file_id")
		job_id.DbValue = RsRow("job_id")
		job_file_name.DbValue = RsRow("job_file_name")
		job_file_title.DbValue = RsRow("job_file_title")
	End Sub
	...
	Sub RenderListRow()
		...
		job_file_id.ViewValue = job_file_id.CurrentValue
		job_file_id.ViewCustomAttributes = ""
		...
		' job_file_name
		job_file_name.ViewValue = job_file_name.CurrentValue
		job_file_name.ViewCustomAttributes = ""

		' job_file_title
		job_file_title.ViewValue = job_file_title.CurrentValue
		job_file_title.ViewCustomAttributes = ""

		' job_file_id
		job_file_id.LinkCustomAttributes = ""
		job_file_id.HrefValue = ""
		job_file_id.TooltipValue = ""
		...
		' job_file_name
		job_file_name.LinkCustomAttributes = ""
		job_file_name.HrefValue = ""
		job_file_name.TooltipValue = ""

		' job_file_title
		job_file_title.LinkCustomAttributes = ""
		job_file_title.HrefValue = ""
		job_file_title.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [job_file_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("job_file_id", job_file_id.ExportValue(Export))
					Call XmlDoc.AddField("job_id", job_id.ExportValue(Export))
					Call XmlDoc.AddField("job_file_name", job_file_name.ExportValue(Export))
					Call XmlDoc.AddField("job_file_title", job_file_title.ExportValue(Export))
				Else
					Call XmlDoc.AddField("job_file_id", job_file_id.ExportValue(Export))
					Call XmlDoc.AddField("job_id", job_id.ExportValue(Export))
					Call XmlDoc.AddField("job_file_name", job_file_name.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If job_file_id.Exportable Then Call Doc.ExportCaption(job_file_id)
				If job_id.Exportable Then Call Doc.ExportCaption(job_id)
				If job_file_name.Exportable Then Call Doc.ExportCaption(job_file_name)
				If job_file_title.Exportable Then Call Doc.ExportCaption(job_file_title)
			Else
				If job_file_id.Exportable Then Call Doc.ExportCaption(job_file_id)
				If job_id.Exportable Then Call Doc.ExportCaption(job_id)
				If job_file_name.Exportable Then Call Doc.ExportCaption(job_file_name)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If job_file_id.Exportable Then Call Doc.ExportField(job_file_id)
					If job_id.Exportable Then Call Doc.ExportField(job_id)
					If job_file_name.Exportable Then Call Doc.ExportField(job_file_name)
					If job_file_title.Exportable Then Call Doc.ExportField(job_file_title)
				Else
					If job_file_id.Exportable Then Call Doc.ExportField(job_file_id)
					If job_id.Exportable Then Call Doc.ExportField(job_id)
					If job_file_name.Exportable Then Call Doc.ExportField(job_file_name)
...
'			End If
	...
	' ----------------
	' Field job_file_id
	Private m_job_file_id

	Public Property Get job_file_id()
		If Not IsObject(m_job_file_id) Then
			Set m_job_file_id = NewFldObj("job_file_th", "job_file_th", "x_job_file_id", "job_file_id", "[job_file_id]", "[job_file_id]", 3, 0, "[job_file_id]", False, False, FALSE, "FORMATTED TEXT")
			m_job_file_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set job_file_id = m_job_file_id
	End Property
	...
	Public Property Get job_id()
		If Not IsObject(m_job_id) Then
			Set m_job_id = NewFldObj("job_file_th", "job_file_th", "x_job_id", "job_id", "[job_id]", "[job_id]", 3, 0, "[job_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	' Field job_file_name
	Private m_job_file_name

	Public Property Get job_file_name()
		If Not IsObject(m_job_file_name) Then
			Set m_job_file_name = NewFldObj("job_file_th", "job_file_th", "x_job_file_name", "job_file_name", "[job_file_name]", "[job_file_name]", 202, 0, "[job_file_name]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set job_file_name = m_job_file_name
	End Property

	' Field job_file_title
	Private m_job_file_title

	Public Property Get job_file_title()
		If Not IsObject(m_job_file_title) Then
			Set m_job_file_title = NewFldObj("job_file_th", "job_file_th", "x_job_file_title", "job_file_title", "[job_file_title]", "[job_file_title]", 202, 0, "[job_file_title]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set job_file_title = m_job_file_title
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_job_file_id) Then Set m_job_file_id = Nothing
		If IsObject(m_job_id) Then Set m_job_id = Nothing
		If IsObject(m_job_file_name) Then Set m_job_file_name = Nothing
		If IsObject(m_job_file_title) Then Set m_job_file_title = Nothing
