...
' ASPMaker configuration for Table news_sale
Dim news_sale
...
Class cnews_sale
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "news_sale_id", news_sale_id)
		Call ew_SetArObj(Fields, "news_sale_pdf", news_sale_pdf)
		Call ew_SetArObj(Fields, "news_sale_title", news_sale_title)
	...
	Public Property Get HighlightName()
		HighlightName = "news_sale_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [news_sale]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[news_sale_id] = @news_sale_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(news_sale_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@news_sale_id@", ew_AdjustSql(news_sale_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_news_salelist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_news_salelist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_news_saleview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_news_saleview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_news_saleedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_news_saleadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_news_saledelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(news_sale_id.CurrentValue) Then
			sUrl = sUrl & "news_sale_id=" & news_sale_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("news_sale_id") ' news_sale_id
	...
	Public Sub LoadListRowValues(RsRow)
		news_sale_id.DbValue = RsRow("news_sale_id")
		news_sale_pdf.DbValue = RsRow("news_sale_pdf")
		news_sale_title.DbValue = RsRow("news_sale_title")
	...
	Sub RenderListRow()
		...
		news_sale_id.ViewValue = news_sale_id.CurrentValue
		news_sale_id.ViewCustomAttributes = ""

		' news_sale_pdf
		news_sale_pdf.ViewValue = news_sale_pdf.CurrentValue
		news_sale_pdf.ViewCustomAttributes = ""

		' news_sale_title
		news_sale_title.ViewValue = news_sale_title.CurrentValue
		news_sale_title.ViewCustomAttributes = ""
		...
		' news_sale_id
		news_sale_id.LinkCustomAttributes = ""
		news_sale_id.HrefValue = ""
		news_sale_id.TooltipValue = ""

		' news_sale_pdf
		news_sale_pdf.LinkCustomAttributes = ""
		news_sale_pdf.HrefValue = ""
		news_sale_pdf.TooltipValue = ""

		' news_sale_title
		news_sale_title.LinkCustomAttributes = ""
		news_sale_title.HrefValue = ""
		news_sale_title.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [news_sale] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("news_sale_id", news_sale_id.ExportValue(Export))
					Call XmlDoc.AddField("news_sale_pdf", news_sale_pdf.ExportValue(Export))
					Call XmlDoc.AddField("news_sale_title", news_sale_title.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If news_sale_id.Exportable Then Call Doc.ExportCaption(news_sale_id)
				If news_sale_pdf.Exportable Then Call Doc.ExportCaption(news_sale_pdf)
				If news_sale_title.Exportable Then Call Doc.ExportCaption(news_sale_title)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If news_sale_id.Exportable Then Call Doc.ExportField(news_sale_id)
					If news_sale_pdf.Exportable Then Call Doc.ExportField(news_sale_pdf)
					If news_sale_title.Exportable Then Call Doc.ExportField(news_sale_title)
...
'			End If
	...
	' ----------------
	' Field news_sale_id
	Private m_news_sale_id

	Public Property Get news_sale_id()
		If Not IsObject(m_news_sale_id) Then
			Set m_news_sale_id = NewFldObj("news_sale", "news_sale", "x_news_sale_id", "news_sale_id", "[news_sale_id]", "[news_sale_id]", 3, 0, "[news_sale_id]", False, False, FALSE, "FORMATTED TEXT")
			m_news_sale_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set news_sale_id = m_news_sale_id
	End Property

	' Field news_sale_pdf
	Private m_news_sale_pdf

	Public Property Get news_sale_pdf()
		If Not IsObject(m_news_sale_pdf) Then
			Set m_news_sale_pdf = NewFldObj("news_sale", "news_sale", "x_news_sale_pdf", "news_sale_pdf", "[news_sale_pdf]", "[news_sale_pdf]", 202, 0, "[news_sale_pdf]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set news_sale_pdf = m_news_sale_pdf
	End Property

	' Field news_sale_title
	Private m_news_sale_title

	Public Property Get news_sale_title()
		If Not IsObject(m_news_sale_title) Then
			Set m_news_sale_title = NewFldObj("news_sale", "news_sale", "x_news_sale_title", "news_sale_title", "[news_sale_title]", "[news_sale_title]", 202, 0, "[news_sale_title]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set news_sale_title = m_news_sale_title
	End Property
	...
	Public Property Get start_date()
		If Not IsObject(m_start_date) Then
			Set m_start_date = NewFldObj("news_sale", "news_sale", "x_start_date", "start_date", "[start_date]", "FORMAT([start_date], '')", 135, 8, "[start_date]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get end_date()
		If Not IsObject(m_end_date) Then
			Set m_end_date = NewFldObj("news_sale", "news_sale", "x_end_date", "end_date", "[end_date]", "FORMAT([end_date], '')", 135, 8, "[end_date]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Private Sub Class_Terminate
		If IsObject(m_news_sale_id) Then Set m_news_sale_id = Nothing
		If IsObject(m_news_sale_pdf) Then Set m_news_sale_pdf = Nothing
		If IsObject(m_news_sale_title) Then Set m_news_sale_title = Nothing
