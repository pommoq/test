...
' Define page object
Dim job_th_view
Set job_th_view = New cjob_th_view
Set Page = job_th_view

' Page init processing
job_th_view.Page_Init()

' Page main processing
job_th_view.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
job_th_view.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<% If job_th.Export = "" Then %>
<script type="text/javascript">
// Page object
var job_th_view = new ew_Page("job_th_view");
job_th_view.PageID = "view"; // Page ID
var EW_PAGE_ID = job_th_view.PageID; // For backward compatibility
// Form object
var fjob_thview = new ew_Form("fjob_thview");
// Form_CustomValidate event
fjob_thview.Form_CustomValidate = 
...
fjob_thview.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fjob_thview.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% job_th_view.ExportOptions.Render "body", "", "", "", "", "" %>
<% If Not job_th_view.ExportOptions.UseDropDownButton Then %>
</div>
<div class="ewViewOtherOptions">
<% End If %>
<%
	job_th_view.ActionOptions.Render "body", "", "", "", "", ""
	job_th_view.DetailOptions.Render "body", "", "", "", "", ""
%>
</div>
<% End If %>
<% job_th_view.ShowPageHeader() %>
<% job_th_view.ShowMessage %>
<form name="fjob_thview" id="fjob_thview" class="ewForm form-inline" action="<%= ew_CurrentPage %>" method="post">
<input type="hidden" name="t" value="job_th">
<table class="ewGrid"><tr><td>
<table id="tbl_job_thview" class="table table-bordered table-striped">
...
<span<%= job_th.job_intro.ViewAttributes %>>
<%= job_th.job_intro.ViewValue %>
</span>
</span>
</td>
	</tr>
...
<span<%= job_th.job_detail.ViewAttributes %>>
<%= job_th.job_detail.ViewValue %>
</span>
</span>
</td>
	</tr>
...
Class cjob_th_view
	...
	Private Sub Class_Initialize()
		...
		If Request.QueryString("job_id").Count > 0 Then
			ew_AddKey RecKey, "job_id", Request.QueryString("job_id")
			KeyUrl = KeyUrl & "&amp;job_id=" & Server.URLEncode(Request.QueryString("job_id"))
		End If
	...
	Sub Page_Main()
		...
		If IsPageRequest Then ' Validate request
			...
			Else
				sReturnUrl = "pom_job_thlist.asp" ' Return to list
			End If

			' Get action
			job_th.CurrentAction = "I" ' Display form
			Select Case job_th.CurrentAction
				Case "I" ' Get a record to display
					If Not LoadRow() Then ' Load record based on key
						...
						End If
						sReturnUrl = "pom_job_thlist.asp" ' No matching record, return to list
					End If
			End Select
		Else
			sReturnUrl = "pom_job_thlist.asp" ' Not page request, return to list
		End If
		If sReturnUrl <> "" Then Call Page_Terminate(sReturnUrl)

		' Render row
		job_th.RowType = EW_ROWTYPE_VIEW
