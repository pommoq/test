...
' ASPMaker configuration for Table e_library_th
Dim e_library_th
...
Class ce_library_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "el_id", el_id)
		Call ew_SetArObj(Fields, "el_date", el_date)
		Call ew_SetArObj(Fields, "el_title", el_title)
		Call ew_SetArObj(Fields, "el_pdf", el_pdf)
		Call ew_SetArObj(Fields, "el_img", el_img)
		Call ew_SetArObj(Fields, "el_detail", el_detail)
		Call ew_SetArObj(Fields, "el_create", el_create)
		Call ew_SetArObj(Fields, "el_update", el_update)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "e_library_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [e_library_th]"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(el_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@el_id@", ew_AdjustSql(el_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_e_library_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_e_library_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_e_library_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_e_library_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_e_library_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_e_library_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_e_library_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(el_id.CurrentValue) Then
			sUrl = sUrl & "el_id=" & el_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("el_id") ' el_id
	...
	Public Sub LoadListRowValues(RsRow)
		el_id.DbValue = RsRow("el_id")
		el_date.DbValue = RsRow("el_date")
		el_title.DbValue = RsRow("el_title")
		el_pdf.DbValue = RsRow("el_pdf")
		el_img.DbValue = RsRow("el_img")
		el_detail.DbValue = RsRow("el_detail")
		el_create.DbValue = RsRow("el_create")
		el_update.DbValue = RsRow("el_update")
	End Sub
	...
	Sub RenderListRow()
		...
		el_id.ViewValue = el_id.CurrentValue
		el_id.ViewCustomAttributes = ""

		' el_date
		el_date.ViewValue = el_date.CurrentValue
		el_date.ViewCustomAttributes = ""

		' el_title
		el_title.ViewValue = el_title.CurrentValue
		el_title.ViewCustomAttributes = ""

		' el_pdf
		el_pdf.ViewValue = el_pdf.CurrentValue
		el_pdf.ViewCustomAttributes = ""

		' el_img
		el_img.ViewValue = el_img.CurrentValue
		el_img.ViewCustomAttributes = ""

		' el_detail
		el_detail.ViewValue = el_detail.CurrentValue
		el_detail.ViewCustomAttributes = ""

		' el_create
		el_create.ViewValue = el_create.CurrentValue
		el_create.ViewCustomAttributes = ""

		' el_update
		el_update.ViewValue = el_update.CurrentValue
		el_update.ViewCustomAttributes = ""

		' el_id
		el_id.LinkCustomAttributes = ""
		el_id.HrefValue = ""
		el_id.TooltipValue = ""

		' el_date
		el_date.LinkCustomAttributes = ""
		el_date.HrefValue = ""
		el_date.TooltipValue = ""

		' el_title
		el_title.LinkCustomAttributes = ""
		el_title.HrefValue = ""
		el_title.TooltipValue = ""

		' el_pdf
		el_pdf.LinkCustomAttributes = ""
		el_pdf.HrefValue = ""
		el_pdf.TooltipValue = ""

		' el_img
		el_img.LinkCustomAttributes = ""
		el_img.HrefValue = ""
		el_img.TooltipValue = ""

		' el_detail
		el_detail.LinkCustomAttributes = ""
		el_detail.HrefValue = ""
		el_detail.TooltipValue = ""

		' el_create
		el_create.LinkCustomAttributes = ""
		el_create.HrefValue = ""
		el_create.TooltipValue = ""

		' el_update
		el_update.LinkCustomAttributes = ""
		el_update.HrefValue = ""
		el_update.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [e_library_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("el_id", el_id.ExportValue(Export))
					Call XmlDoc.AddField("el_date", el_date.ExportValue(Export))
					Call XmlDoc.AddField("el_title", el_title.ExportValue(Export))
					Call XmlDoc.AddField("el_pdf", el_pdf.ExportValue(Export))
					Call XmlDoc.AddField("el_img", el_img.ExportValue(Export))
					Call XmlDoc.AddField("el_detail", el_detail.ExportValue(Export))
					Call XmlDoc.AddField("el_create", el_create.ExportValue(Export))
					Call XmlDoc.AddField("el_update", el_update.ExportValue(Export))
				Else
					Call XmlDoc.AddField("el_id", el_id.ExportValue(Export))
					Call XmlDoc.AddField("el_date", el_date.ExportValue(Export))
					Call XmlDoc.AddField("el_title", el_title.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If el_id.Exportable Then Call Doc.ExportCaption(el_id)
				If el_date.Exportable Then Call Doc.ExportCaption(el_date)
				If el_title.Exportable Then Call Doc.ExportCaption(el_title)
				If el_pdf.Exportable Then Call Doc.ExportCaption(el_pdf)
				If el_img.Exportable Then Call Doc.ExportCaption(el_img)
				If el_detail.Exportable Then Call Doc.ExportCaption(el_detail)
				If el_create.Exportable Then Call Doc.ExportCaption(el_create)
				If el_update.Exportable Then Call Doc.ExportCaption(el_update)
			Else
				If el_id.Exportable Then Call Doc.ExportCaption(el_id)
				If el_date.Exportable Then Call Doc.ExportCaption(el_date)
				If el_title.Exportable Then Call Doc.ExportCaption(el_title)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If el_id.Exportable Then Call Doc.ExportField(el_id)
					If el_date.Exportable Then Call Doc.ExportField(el_date)
					If el_title.Exportable Then Call Doc.ExportField(el_title)
					If el_pdf.Exportable Then Call Doc.ExportField(el_pdf)
					If el_img.Exportable Then Call Doc.ExportField(el_img)
					If el_detail.Exportable Then Call Doc.ExportField(el_detail)
					If el_create.Exportable Then Call Doc.ExportField(el_create)
					If el_update.Exportable Then Call Doc.ExportField(el_update)
				Else
					If el_id.Exportable Then Call Doc.ExportField(el_id)
					If el_date.Exportable Then Call Doc.ExportField(el_date)
					If el_title.Exportable Then Call Doc.ExportField(el_title)
...
'			End If
	...
	Public Property Get el_id()
		If Not IsObject(m_el_id) Then
			Set m_el_id = NewFldObj("e_library_th", "e_library_th", "x_el_id", "el_id", "[el_id]", "[el_id]", 3, 0, "[el_id]", False, False, FALSE, "FORMATTED TEXT")
			m_el_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set el_id = m_el_id
	End Property
	...
	Public Property Get el_date()
		If Not IsObject(m_el_date) Then
			Set m_el_date = NewFldObj("e_library_th", "e_library_th", "x_el_date", "el_date", "[el_date]", "FORMAT([el_date], '')", 135, 8, "[el_date]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_date = m_el_date
	End Property

	' Field el_title
	Private m_el_title

	Public Property Get el_title()
		If Not IsObject(m_el_title) Then
			Set m_el_title = NewFldObj("e_library_th", "e_library_th", "x_el_title", "el_title", "[el_title]", "[el_title]", 202, 0, "[el_title]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_title = m_el_title
	End Property

	' Field el_pdf
	Private m_el_pdf

	Public Property Get el_pdf()
		If Not IsObject(m_el_pdf) Then
			Set m_el_pdf = NewFldObj("e_library_th", "e_library_th", "x_el_pdf", "el_pdf", "[el_pdf]", "[el_pdf]", 202, 0, "[el_pdf]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_pdf = m_el_pdf
	End Property
	...
	Public Property Get el_img()
		If Not IsObject(m_el_img) Then
			Set m_el_img = NewFldObj("e_library_th", "e_library_th", "x_el_img", "el_img", "[el_img]", "[el_img]", 202, 0, "[el_img]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_img = m_el_img
	End Property
	...
	Public Property Get el_detail()
		If Not IsObject(m_el_detail) Then
			Set m_el_detail = NewFldObj("e_library_th", "e_library_th", "x_el_detail", "el_detail", "[el_detail]", "[el_detail]", 203, 0, "[el_detail]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_detail = m_el_detail
	End Property

	' Field el_create
	Private m_el_create

	Public Property Get el_create()
		If Not IsObject(m_el_create) Then
			Set m_el_create = NewFldObj("e_library_th", "e_library_th", "x_el_create", "el_create", "[el_create]", "[el_create]", 202, 0, "[el_create]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_create = m_el_create
	End Property

	' Field el_update
	Private m_el_update

	Public Property Get el_update()
		If Not IsObject(m_el_update) Then
			Set m_el_update = NewFldObj("e_library_th", "e_library_th", "x_el_update", "el_update", "[el_update]", "[el_update]", 202, 0, "[el_update]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set el_update = m_el_update
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_el_id) Then Set m_el_id = Nothing
		If IsObject(m_el_date) Then Set m_el_date = Nothing
		If IsObject(m_el_title) Then Set m_el_title = Nothing
		If IsObject(m_el_pdf) Then Set m_el_pdf = Nothing
		If IsObject(m_el_img) Then Set m_el_img = Nothing
		If IsObject(m_el_detail) Then Set m_el_detail = Nothing
		If IsObject(m_el_create) Then Set m_el_create = Nothing
		If IsObject(m_el_update) Then Set m_el_update = Nothing
