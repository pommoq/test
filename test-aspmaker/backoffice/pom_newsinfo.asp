...
' ASPMaker configuration for Table news
Dim news
...
Class cnews
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_newslist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_newslist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_newsview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_newsview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_newsedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_newsadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_newsdelete.asp", UrlParm(""))
	End Function
	...
	Public Sub LoadListRowValues(RsRow)
		...
		news_img.Upload.DbValue = RsRow("news_img")
	...
	Sub RenderListRow()
		...
		' news_img
		news_img.UploadPath = "./Upload/news"
		If Not ew_Empty(news_img.Upload.DbValue) Then
			news_img.ViewValue = news_img.Upload.DbValue
			news_img.ImageAlt = news_img.FldAlt
			news_img.ViewValue = ew_UploadPathEx(False, news_img.UploadPath) & news_img.Upload.DbValue
		Else
			news_img.ViewValue = ""
		End If
		...
		news_img.HrefValue = ""
		news_img.HrefValue2 = news_img.UploadPath & news_img.Upload.DbValue
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [news] WHERE " & sWhereList
			...
			Do While Not rsfile.Eof
				...
				news_img.UploadPath = "./Upload/news"
				If Not ew_Empty(news_img.Upload.DbValue) Then
					ew_DeleteFile ew_UploadPathEx(True, news_img.UploadPath) & news_img.Upload.DbValue
				End If
				rsfile.MoveNext
			Loop
			rsfile.Close
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("news_img", news_img.Upload.DbValue)
...
'			End If
	...
	Public Property Get news_id()
		If Not IsObject(m_news_id) Then
			Set m_news_id = NewFldObj("news", "news", "x_news_id", "news_id", "[news_id]", "[news_id]", 3, 0, "[news_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	Public Property Get news_img()
		If Not IsObject(m_news_img) Then
			Set m_news_img = NewFldObj("news", "news", "x_news_img", "news_img", "[news_img]", "[news_img]", 202, 0, "[news_img]", False, False, FALSE, "IMAGE")
			m_news_img.UploadPath = EW_UPLOAD_DEST_PATH
		End If
	...
	Public Property Get news_date()
		If Not IsObject(m_news_date) Then
			Set m_news_date = NewFldObj("news", "news", "x_news_date", "news_date", "[news_date]", "FORMAT([news_date], '')", 135, 8, "[news_date]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_category()
		If Not IsObject(m_news_category) Then
			Set m_news_category = NewFldObj("news", "news", "x_news_category", "news_category", "[news_category]", "[news_category]", 202, 0, "[news_category]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_category_sub()
		If Not IsObject(m_news_category_sub) Then
			Set m_news_category_sub = NewFldObj("news", "news", "x_news_category_sub", "news_category_sub", "[news_category_sub]", "[news_category_sub]", 202, 0, "[news_category_sub]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get start_date()
		If Not IsObject(m_start_date) Then
			Set m_start_date = NewFldObj("news", "news", "x_start_date", "start_date", "[start_date]", "FORMAT([start_date], '')", 135, 8, "[start_date]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get end_date()
		If Not IsObject(m_end_date) Then
			Set m_end_date = NewFldObj("news", "news", "x_end_date", "end_date", "[end_date]", "FORMAT([end_date], '')", 135, 8, "[end_date]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_pdf()
		If Not IsObject(m_news_pdf) Then
			Set m_news_pdf = NewFldObj("news", "news", "x_news_pdf", "news_pdf", "[news_pdf]", "[news_pdf]", 202, 0, "[news_pdf]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_subject()
		If Not IsObject(m_news_subject) Then
			Set m_news_subject = NewFldObj("news", "news", "x_news_subject", "news_subject", "[news_subject]", "[news_subject]", 202, 0, "[news_subject]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_subject_th()
		If Not IsObject(m_news_subject_th) Then
			Set m_news_subject_th = NewFldObj("news", "news", "x_news_subject_th", "news_subject_th", "[news_subject_th]", "[news_subject_th]", 202, 0, "[news_subject_th]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_intro()
		If Not IsObject(m_news_intro) Then
			Set m_news_intro = NewFldObj("news", "news", "x_news_intro", "news_intro", "[news_intro]", "[news_intro]", 203, 0, "[news_intro]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_intro_th()
		If Not IsObject(m_news_intro_th) Then
			Set m_news_intro_th = NewFldObj("news", "news", "x_news_intro_th", "news_intro_th", "[news_intro_th]", "[news_intro_th]", 203, 0, "[news_intro_th]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_content()
		If Not IsObject(m_news_content) Then
			Set m_news_content = NewFldObj("news", "news", "x_news_content", "news_content", "[news_content]", "[news_content]", 203, 0, "[news_content]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_content_th()
		If Not IsObject(m_news_content_th) Then
			Set m_news_content_th = NewFldObj("news", "news", "x_news_content_th", "news_content_th", "[news_content_th]", "[news_content_th]", 203, 0, "[news_content_th]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_show_en()
		If Not IsObject(m_news_show_en) Then
			Set m_news_show_en = NewFldObj("news", "news", "x_news_show_en", "news_show_en", "[news_show_en]", "[news_show_en]", 202, 0, "[news_show_en]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_show()
		If Not IsObject(m_news_show) Then
			Set m_news_show = NewFldObj("news", "news", "x_news_show", "news_show", "[news_show]", "[news_show]", 202, 0, "[news_show]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_show_home()
		If Not IsObject(m_news_show_home) Then
			Set m_news_show_home = NewFldObj("news", "news", "x_news_show_home", "news_show_home", "[news_show_home]", "[news_show_home]", 202, 0, "[news_show_home]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_create()
		If Not IsObject(m_news_create) Then
			Set m_news_create = NewFldObj("news", "news", "x_news_create", "news_create", "[news_create]", "[news_create]", 202, 0, "[news_create]", False, False, FALSE, "FORMATTED TEXT")
		End If
	...
	Public Property Get news_update()
		If Not IsObject(m_news_update) Then
			Set m_news_update = NewFldObj("news", "news", "x_news_update", "news_update", "[news_update]", "[news_update]", 202, 0, "[news_update]", False, False, FALSE, "FORMATTED TEXT")
		End If
