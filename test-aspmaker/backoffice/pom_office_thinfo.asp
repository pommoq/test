...
' ASPMaker configuration for Table office_th
Dim office_th
...
Class coffice_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "office_name_en", office_name_en)
		Call ew_SetArObj(Fields, "office_name_th", office_name_th)
		Call ew_SetArObj(Fields, "office_sort", office_sort)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "office_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [office_th]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[office_id] = @office_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(office_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@office_id@", ew_AdjustSql(office_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_office_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_office_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_office_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_office_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_office_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_office_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_office_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(office_id.CurrentValue) Then
			sUrl = sUrl & "office_id=" & office_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("office_id") ' office_id
	...
	Public Sub LoadListRowValues(RsRow)
		...
		office_name_en.DbValue = RsRow("office_name_en")
		office_name_th.DbValue = RsRow("office_name_th")
		office_sort.DbValue = RsRow("office_sort")
	End Sub
	...
	Sub RenderListRow()
		...
		' office_name_en
		office_name_en.ViewValue = office_name_en.CurrentValue
		office_name_en.ViewCustomAttributes = ""

		' office_name_th
		office_name_th.ViewValue = office_name_th.CurrentValue
		office_name_th.ViewCustomAttributes = ""

		' office_sort
		office_sort.ViewValue = office_sort.CurrentValue
		office_sort.ViewCustomAttributes = ""
		...
		' office_name_en
		office_name_en.LinkCustomAttributes = ""
		office_name_en.HrefValue = ""
		office_name_en.TooltipValue = ""

		' office_name_th
		office_name_th.LinkCustomAttributes = ""
		office_name_th.HrefValue = ""
		office_name_th.TooltipValue = ""

		' office_sort
		office_sort.LinkCustomAttributes = ""
		office_sort.HrefValue = ""
		office_sort.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [office_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("office_name_en", office_name_en.ExportValue(Export))
					Call XmlDoc.AddField("office_name_th", office_name_th.ExportValue(Export))
					Call XmlDoc.AddField("office_sort", office_sort.ExportValue(Export))
				Else
					Call XmlDoc.AddField("office_id", office_id.ExportValue(Export))
					Call XmlDoc.AddField("office_name_en", office_name_en.ExportValue(Export))
					Call XmlDoc.AddField("office_name_th", office_name_th.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				...
				If office_name_en.Exportable Then Call Doc.ExportCaption(office_name_en)
				If office_name_th.Exportable Then Call Doc.ExportCaption(office_name_th)
				If office_sort.Exportable Then Call Doc.ExportCaption(office_sort)
			Else
				If office_id.Exportable Then Call Doc.ExportCaption(office_id)
				If office_name_en.Exportable Then Call Doc.ExportCaption(office_name_en)
				If office_name_th.Exportable Then Call Doc.ExportCaption(office_name_th)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					If office_name_en.Exportable Then Call Doc.ExportField(office_name_en)
					If office_name_th.Exportable Then Call Doc.ExportField(office_name_th)
					If office_sort.Exportable Then Call Doc.ExportField(office_sort)
				Else
					If office_id.Exportable Then Call Doc.ExportField(office_id)
					If office_name_en.Exportable Then Call Doc.ExportField(office_name_en)
					If office_name_th.Exportable Then Call Doc.ExportField(office_name_th)
...
'			End If
	...
	Public Property Get office_id()
		If Not IsObject(m_office_id) Then
			Set m_office_id = NewFldObj("office_th", "office_th", "x_office_id", "office_id", "[office_id]", "[office_id]", 3, 0, "[office_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	Private m_office_name_en

	Public Property Get office_name_en()
		If Not IsObject(m_office_name_en) Then
			Set m_office_name_en = NewFldObj("office_th", "office_th", "x_office_name_en", "office_name_en", "[office_name_en]", "[office_name_en]", 202, 0, "[office_name_en]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set office_name_en = m_office_name_en
	End Property

	' Field office_name_th
	Private m_office_name_th

	Public Property Get office_name_th()
		If Not IsObject(m_office_name_th) Then
			Set m_office_name_th = NewFldObj("office_th", "office_th", "x_office_name_th", "office_name_th", "[office_name_th]", "[office_name_th]", 202, 0, "[office_name_th]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set office_name_th = m_office_name_th
	End Property

	' Field office_sort
	Private m_office_sort

	Public Property Get office_sort()
		If Not IsObject(m_office_sort) Then
			Set m_office_sort = NewFldObj("office_th", "office_th", "x_office_sort", "office_sort", "[office_sort]", "[office_sort]", 202, 0, "[office_sort]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set office_sort = m_office_sort
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		...
		If IsObject(m_office_name_en) Then Set m_office_name_en = Nothing
		If IsObject(m_office_name_th) Then Set m_office_name_th = Nothing
		If IsObject(m_office_sort) Then Set m_office_sort = Nothing
