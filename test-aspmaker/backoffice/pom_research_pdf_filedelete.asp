...
' Define page object
Dim research_pdf_file_delete
Set research_pdf_file_delete = New cresearch_pdf_file_delete
Set Page = research_pdf_file_delete

' Page init processing
research_pdf_file_delete.Page_Init()

' Page main processing
research_pdf_file_delete.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
research_pdf_file_delete.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var research_pdf_file_delete = new ew_Page("research_pdf_file_delete");
research_pdf_file_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = research_pdf_file_delete.PageID; // For backward compatibility
// Form object
var fresearch_pdf_filedelete = new ew_Form("fresearch_pdf_filedelete");
// Form_CustomValidate event
fresearch_pdf_filedelete.Form_CustomValidate = 
...
fresearch_pdf_filedelete.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fresearch_pdf_filedelete.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
Set research_pdf_file_delete.Recordset = research_pdf_file_delete.LoadRecordset()
research_pdf_file_delete.TotalRecs = research_pdf_file_delete.Recordset.RecordCount ' Get record count
If research_pdf_file_delete.TotalRecs <= 0 Then ' No record found, exit
	research_pdf_file_delete.Recordset.Close
	Set research_pdf_file_delete.Recordset = Nothing
	Call research_pdf_file_delete.Page_Terminate("pom_research_pdf_filelist.asp") ' Return to list
End If
%>
<% If research_pdf_file.Export = "" Then %>
<% Breadcrumb.Render() %>
<% End If %>
<% research_pdf_file_delete.ShowPageHeader() %>
<% research_pdf_file_delete.ShowMessage %>
<form name="fresearch_pdf_filedelete" id="fresearch_pdf_filedelete" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="research_pdf_file">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<% For i = 0 to UBound(research_pdf_file_delete.RecKeys) %>
<input type="hidden" name="key_m" id="key_m" value="<%= ew_HtmlEncode(ew_GetKeyValue(research_pdf_file_delete.RecKeys(i))) %>">
<% Next %>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_research_pdf_filedelete" class="ewTable ewTableSeparate">
<%= research_pdf_file.TableCustomInnerHtml %>
	<thead>
	<tr class="ewTableHeader">
<% If research_pdf_file.rsh_pdf_id.Visible Then ' rsh_pdf_id %>
		<td><span id="elh_research_pdf_file_rsh_pdf_id" class="research_pdf_file_rsh_pdf_id"><%= research_pdf_file.rsh_pdf_id.FldCaption %></span></td>
<% End If %>
<% If research_pdf_file.rsh_id.Visible Then ' rsh_id %>
		<td><span id="elh_research_pdf_file_rsh_id" class="research_pdf_file_rsh_id"><%= research_pdf_file.rsh_id.FldCaption %></span></td>
<% End If %>
<% If research_pdf_file.rsh_pdf_file.Visible Then ' rsh_pdf_file %>
		<td><span id="elh_research_pdf_file_rsh_pdf_file" class="research_pdf_file_rsh_pdf_file"><%= research_pdf_file.rsh_pdf_file.FldCaption %></span></td>
<% End If %>
<% If research_pdf_file.rsh_pdf_title.Visible Then ' rsh_pdf_title %>
		<td><span id="elh_research_pdf_file_rsh_pdf_title" class="research_pdf_file_rsh_pdf_title"><%= research_pdf_file.rsh_pdf_title.FldCaption %></span></td>
<% End If %>
	</tr>
	</thead>
	<tbody>
<%
research_pdf_file_delete.RecCnt = 0
research_pdf_file_delete.RowCnt = 0
Do While (Not research_pdf_file_delete.Recordset.Eof)
	research_pdf_file_delete.RecCnt = research_pdf_file_delete.RecCnt + 1
	research_pdf_file_delete.RowCnt = research_pdf_file_delete.RowCnt + 1

	' Set row properties
	Call research_pdf_file.ResetAttrs()
	research_pdf_file.RowType = EW_ROWTYPE_VIEW ' view

	' Get the field contents
	Call research_pdf_file_delete.LoadRowValues(research_pdf_file_delete.Recordset)

	' Render row
	Call research_pdf_file_delete.RenderRow()
%>
	<tr<%= research_pdf_file.RowAttributes %>>
<% If research_pdf_file.rsh_pdf_id.Visible Then ' rsh_pdf_id %>
		<td<%= research_pdf_file.rsh_pdf_id.CellAttributes %>>
<span id="el<%= research_pdf_file_delete.RowCnt %>_research_pdf_file_rsh_pdf_id" class="control-group research_pdf_file_rsh_pdf_id">
...
<span id="el<%= research_pdf_file_delete.RowCnt %>_research_pdf_file_rsh_id" class="control-group research_pdf_file_rsh_id">
...
<span id="el<%= research_pdf_file_delete.RowCnt %>_research_pdf_file_rsh_pdf_file" class="control-group research_pdf_file_rsh_pdf_file">
...
<span id="el<%= research_pdf_file_delete.RowCnt %>_research_pdf_file_rsh_pdf_title" class="control-group research_pdf_file_rsh_pdf_title">
...
<%
	research_pdf_file_delete.Recordset.MoveNext
Loop
research_pdf_file_delete.Recordset.Close
Set research_pdf_file_delete.Recordset = Nothing
%>
...
<script type="text/javascript">
fresearch_pdf_filedelete.Init();
</script>
<%
research_pdf_file_delete.ShowPageFooter()
...
' Drop page object
Set research_pdf_file_delete = Nothing
%>
<%
...
' Page Class
'
Class cresearch_pdf_file_delete
	...
	Public Property Get PageObjName()
		PageObjName = "research_pdf_file_delete"
	End Property
	...
	Sub Page_Main()
		...
		' Load Key Parameters
		RecKeys = research_pdf_file.GetRecordKeys() ' Load record keys
		sFilter = research_pdf_file.GetKeyFilter()
		If sFilter = "" Then
			Call Page_Terminate("pom_research_pdf_filelist.asp") ' Prevent SQL injection, return to list
		End If

		' Set up filter (Sql Where Clause) and get Return Sql
		' Sql constructor in research_pdf_file class, research_pdf_fileinfo.asp

		research_pdf_file.CurrentFilter = sFilter

		' Get action
		If Request.Form("a_delete").Count > 0 Then
			research_pdf_file.CurrentAction = Request.Form("a_delete")
		Else
			research_pdf_file.CurrentAction = "I"	' Display record
		End If
		Select Case research_pdf_file.CurrentAction
			Case "D" ' Delete
				research_pdf_file.SendEmail = True ' Send email on delete success
				If DeleteRows() Then ' Delete rows
					SuccessMessage = Language.Phrase("DeleteSuccess") ' Set up success message
					Call Page_Terminate(research_pdf_file.ReturnUrl) ' Return to caller
				End If
		End Select
	End Sub
	...
	Function DeleteRows()
		...
		If DeleteRows Then
			...
			Do While Not RsDelete.Eof
				DeleteRows = research_pdf_file.Row_Deleting(RsDelete)
		...
		If DeleteRows Then
			...
			Do While Not RsOld.Eof
				Call research_pdf_file.Row_Deleted(RsOld)
				RsOld.MoveNext
			Loop
		End If
		RsOld.Close
