...
' Define page object
Dim video_delete
Set video_delete = New cvideo_delete
Set Page = video_delete
...
fvideodelete.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fvideodelete.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
Set video_delete.Recordset = video_delete.LoadRecordset()
video_delete.TotalRecs = video_delete.Recordset.RecordCount ' Get record count
If video_delete.TotalRecs <= 0 Then ' No record found, exit
	video_delete.Recordset.Close
	Set video_delete.Recordset = Nothing
	Call video_delete.Page_Terminate("pom_videolist.asp") ' Return to list
End If
%>
<% If video.Export = "" Then %>
<% Breadcrumb.Render() %>
<% End If %>
<% video_delete.ShowPageHeader() %>
<% video_delete.ShowMessage %>
<form name="fvideodelete" id="fvideodelete" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="video">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<% For i = 0 to UBound(video_delete.RecKeys) %>
<input type="hidden" name="key_m" id="key_m" value="<%= ew_HtmlEncode(ew_GetKeyValue(video_delete.RecKeys(i))) %>">
<% Next %>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_videodelete" class="ewTable ewTableSeparate">
<%= video.TableCustomInnerHtml %>
	<thead>
	<tr class="ewTableHeader">
<% If video.video_id.Visible Then ' video_id %>
		<td><span id="elh_video_video_id" class="video_video_id"><%= video.video_id.FldCaption %></span></td>
<% End If %>
<% If video.video_title.Visible Then ' video_title %>
		<td><span id="elh_video_video_title" class="video_video_title"><%= video.video_title.FldCaption %></span></td>
<% End If %>
<% If video.video_link.Visible Then ' video_link %>
		<td><span id="elh_video_video_link" class="video_video_link"><%= video.video_link.FldCaption %></span></td>
<% End If %>
<% If video.video_create.Visible Then ' video_create %>
		<td><span id="elh_video_video_create" class="video_video_create"><%= video.video_create.FldCaption %></span></td>
<% End If %>
<% If video.video_update.Visible Then ' video_update %>
		<td><span id="elh_video_video_update" class="video_video_update"><%= video.video_update.FldCaption %></span></td>
<% End If %>
	</tr>
	</thead>
	<tbody>
...
video_delete.RowCnt = 0
Do While (Not video_delete.Recordset.Eof)
	video_delete.RecCnt = video_delete.RecCnt + 1
	video_delete.RowCnt = video_delete.RowCnt + 1
	...
	Call video_delete.LoadRowValues(video_delete.Recordset)

	' Render row
	Call video_delete.RenderRow()
%>
	<tr<%= video.RowAttributes %>>
<% If video.video_id.Visible Then ' video_id %>
		<td<%= video.video_id.CellAttributes %>>
<span id="el<%= video_delete.RowCnt %>_video_video_id" class="control-group video_video_id">
...
<span id="el<%= video_delete.RowCnt %>_video_video_title" class="control-group video_video_title">
...
<span id="el<%= video_delete.RowCnt %>_video_video_link" class="control-group video_video_link">
...
<span id="el<%= video_delete.RowCnt %>_video_video_create" class="control-group video_video_create">
...
<span id="el<%= video_delete.RowCnt %>_video_video_update" class="control-group video_video_update">
...
Class cvideo_delete
	...
	Sub Page_Main()
		...
		' Load Key Parameters
		RecKeys = video.GetRecordKeys() ' Load record keys
		sFilter = video.GetKeyFilter()
		If sFilter = "" Then
			Call Page_Terminate("pom_videolist.asp") ' Prevent SQL injection, return to list
		End If

		' Set up filter (Sql Where Clause) and get Return Sql
		' Sql constructor in video class, videoinfo.asp

		video.CurrentFilter = sFilter

		' Get action
		If Request.Form("a_delete").Count > 0 Then
			video.CurrentAction = Request.Form("a_delete")
		Else
			video.CurrentAction = "I"	' Display record
		End If
		Select Case video.CurrentAction
			Case "D" ' Delete
				video.SendEmail = True ' Send email on delete success
				If DeleteRows() Then ' Delete rows
					SuccessMessage = Language.Phrase("DeleteSuccess") ' Set up success message
					Call Page_Terminate(video.ReturnUrl) ' Return to caller
				End If
		End Select
	End Sub
	...
	Function DeleteRows()
		...
		If DeleteRows Then
			...
			Do While Not RsDelete.Eof
				DeleteRows = video.Row_Deleting(RsDelete)
