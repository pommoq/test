...
' ASPMaker configuration for Table sys_admin_menu
Dim sys_admin_menu

' Define table class
Class csys_admin_menu
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "sys_admin_menu_id", sys_admin_menu_id)
	...
	Public Property Get HighlightName()
		HighlightName = "sys_admin_menu_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [sys_admin_menu]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[sys_admin_menu_id] = @sys_admin_menu_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(sys_admin_menu_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@sys_admin_menu_id@", ew_AdjustSql(sys_admin_menu_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_sys_admin_menulist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_sys_admin_menulist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_sys_admin_menuview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_sys_admin_menuview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_sys_admin_menuedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_sys_admin_menuadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_sys_admin_menudelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(sys_admin_menu_id.CurrentValue) Then
			sUrl = sUrl & "sys_admin_menu_id=" & sys_admin_menu_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("sys_admin_menu_id") ' sys_admin_menu_id
	...
	Public Function GetKeyFilter()
		...
		If IsArray(arKeys) Then
			For i = 0 to UBound(arKeys)
				...
				sys_admin_menu_id.CurrentValue = key
	...
	Public Sub LoadListRowValues(RsRow)
		sys_admin_menu_id.DbValue = RsRow("sys_admin_menu_id")
	...
	Sub RenderListRow()
		...
		sys_admin_menu_id.ViewValue = sys_admin_menu_id.CurrentValue
		sys_admin_menu_id.ViewCustomAttributes = ""
		...
		' sys_admin_menu_id
		sys_admin_menu_id.LinkCustomAttributes = ""
		sys_admin_menu_id.HrefValue = ""
		sys_admin_menu_id.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [sys_admin_menu] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("sys_admin_menu_id", sys_admin_menu_id.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If sys_admin_menu_id.Exportable Then Call Doc.ExportCaption(sys_admin_menu_id)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If sys_admin_menu_id.Exportable Then Call Doc.ExportField(sys_admin_menu_id)
...
'			End If
	...
	Private m_sys_admin_menu_id

	Public Property Get sys_admin_menu_id()
		If Not IsObject(m_sys_admin_menu_id) Then
			Set m_sys_admin_menu_id = NewFldObj("sys_admin_menu", "sys_admin_menu", "x_sys_admin_menu_id", "sys_admin_menu_id", "[sys_admin_menu_id]", "[sys_admin_menu_id]", 3, 0, "[sys_admin_menu_id]", False, False, FALSE, "FORMATTED TEXT")
			m_sys_admin_menu_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set sys_admin_menu_id = m_sys_admin_menu_id
	End Property
	...
	Public Property Get admin_id()
		If Not IsObject(m_admin_id) Then
			Set m_admin_id = NewFldObj("sys_admin_menu", "sys_admin_menu", "x_admin_id", "admin_id", "[admin_id]", "[admin_id]", 3, 0, "[admin_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	Public Property Get menu_id()
		If Not IsObject(m_menu_id) Then
			Set m_menu_id = NewFldObj("sys_admin_menu", "sys_admin_menu", "x_menu_id", "menu_id", "[menu_id]", "[menu_id]", 3, 0, "[menu_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	Private Sub Class_Terminate
		If IsObject(m_sys_admin_menu_id) Then Set m_sys_admin_menu_id = Nothing
