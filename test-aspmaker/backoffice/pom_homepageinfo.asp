...
' ASPMaker configuration for Table homepage
Dim homepage
...
Class chomepage
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "hp_id", hp_id)
		Call ew_SetArObj(Fields, "hp_img", hp_img)
		Call ew_SetArObj(Fields, "hp_content", hp_content)
		Call ew_SetArObj(Fields, "hp_show", hp_show)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "homepage_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [homepage]"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(hp_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@hp_id@", ew_AdjustSql(hp_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_homepagelist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_homepagelist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_homepageview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_homepageview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_homepageedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_homepageadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_homepagedelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(hp_id.CurrentValue) Then
			sUrl = sUrl & "hp_id=" & hp_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("hp_id") ' hp_id
	...
	Public Sub LoadListRowValues(RsRow)
		hp_id.DbValue = RsRow("hp_id")
		hp_img.DbValue = RsRow("hp_img")
		hp_content.DbValue = RsRow("hp_content")
		hp_show.DbValue = RsRow("hp_show")
	End Sub
	...
	Sub RenderListRow()
		...
		hp_id.ViewValue = hp_id.CurrentValue
		hp_id.ViewCustomAttributes = ""

		' hp_img
		hp_img.ViewValue = hp_img.CurrentValue
		hp_img.ViewCustomAttributes = ""

		' hp_content
		hp_content.ViewValue = hp_content.CurrentValue
		hp_content.ViewCustomAttributes = ""

		' hp_show
		hp_show.ViewValue = hp_show.CurrentValue
		hp_show.ViewCustomAttributes = ""

		' hp_id
		hp_id.LinkCustomAttributes = ""
		hp_id.HrefValue = ""
		hp_id.TooltipValue = ""

		' hp_img
		hp_img.LinkCustomAttributes = ""
		hp_img.HrefValue = ""
		hp_img.TooltipValue = ""

		' hp_content
		hp_content.LinkCustomAttributes = ""
		hp_content.HrefValue = ""
		hp_content.TooltipValue = ""

		' hp_show
		hp_show.LinkCustomAttributes = ""
		hp_show.HrefValue = ""
		hp_show.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [homepage] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("hp_id", hp_id.ExportValue(Export))
					Call XmlDoc.AddField("hp_img", hp_img.ExportValue(Export))
					Call XmlDoc.AddField("hp_content", hp_content.ExportValue(Export))
					Call XmlDoc.AddField("hp_show", hp_show.ExportValue(Export))
				Else
					Call XmlDoc.AddField("hp_id", hp_id.ExportValue(Export))
					Call XmlDoc.AddField("hp_img", hp_img.ExportValue(Export))
					Call XmlDoc.AddField("hp_content", hp_content.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If hp_id.Exportable Then Call Doc.ExportCaption(hp_id)
				If hp_img.Exportable Then Call Doc.ExportCaption(hp_img)
				If hp_content.Exportable Then Call Doc.ExportCaption(hp_content)
				If hp_show.Exportable Then Call Doc.ExportCaption(hp_show)
			Else
				If hp_id.Exportable Then Call Doc.ExportCaption(hp_id)
				If hp_img.Exportable Then Call Doc.ExportCaption(hp_img)
				If hp_content.Exportable Then Call Doc.ExportCaption(hp_content)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If hp_id.Exportable Then Call Doc.ExportField(hp_id)
					If hp_img.Exportable Then Call Doc.ExportField(hp_img)
					If hp_content.Exportable Then Call Doc.ExportField(hp_content)
					If hp_show.Exportable Then Call Doc.ExportField(hp_show)
				Else
					If hp_id.Exportable Then Call Doc.ExportField(hp_id)
					If hp_img.Exportable Then Call Doc.ExportField(hp_img)
					If hp_content.Exportable Then Call Doc.ExportField(hp_content)
...
'			End If
	...
	Public Property Get hp_id()
		If Not IsObject(m_hp_id) Then
			Set m_hp_id = NewFldObj("homepage", "homepage", "x_hp_id", "hp_id", "[hp_id]", "[hp_id]", 3, 0, "[hp_id]", False, False, FALSE, "FORMATTED TEXT")
			m_hp_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set hp_id = m_hp_id
	End Property
	...
	Public Property Get hp_img()
		If Not IsObject(m_hp_img) Then
			Set m_hp_img = NewFldObj("homepage", "homepage", "x_hp_img", "hp_img", "[hp_img]", "[hp_img]", 202, 0, "[hp_img]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set hp_img = m_hp_img
	End Property

	' Field hp_content
	Private m_hp_content

	Public Property Get hp_content()
		If Not IsObject(m_hp_content) Then
			Set m_hp_content = NewFldObj("homepage", "homepage", "x_hp_content", "hp_content", "[hp_content]", "[hp_content]", 202, 0, "[hp_content]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set hp_content = m_hp_content
	End Property

	' Field hp_show
	Private m_hp_show

	Public Property Get hp_show()
		If Not IsObject(m_hp_show) Then
			Set m_hp_show = NewFldObj("homepage", "homepage", "x_hp_show", "hp_show", "[hp_show]", "[hp_show]", 202, 0, "[hp_show]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set hp_show = m_hp_show
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_hp_id) Then Set m_hp_id = Nothing
		If IsObject(m_hp_img) Then Set m_hp_img = Nothing
		If IsObject(m_hp_content) Then Set m_hp_content = Nothing
		If IsObject(m_hp_show) Then Set m_hp_show = Nothing
