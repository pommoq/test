...
' Define page object
Dim e_library_th_delete
Set e_library_th_delete = New ce_library_th_delete
Set Page = e_library_th_delete

' Page init processing
e_library_th_delete.Page_Init()

' Page main processing
e_library_th_delete.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
e_library_th_delete.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var e_library_th_delete = new ew_Page("e_library_th_delete");
e_library_th_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = e_library_th_delete.PageID; // For backward compatibility
// Form object
var fe_library_thdelete = new ew_Form("fe_library_thdelete");
// Form_CustomValidate event
fe_library_thdelete.Form_CustomValidate = 
...
fe_library_thdelete.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fe_library_thdelete.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
Set e_library_th_delete.Recordset = e_library_th_delete.LoadRecordset()
e_library_th_delete.TotalRecs = e_library_th_delete.Recordset.RecordCount ' Get record count
If e_library_th_delete.TotalRecs <= 0 Then ' No record found, exit
	e_library_th_delete.Recordset.Close
	Set e_library_th_delete.Recordset = Nothing
	Call e_library_th_delete.Page_Terminate("pom_e_library_thlist.asp") ' Return to list
End If
%>
<% If e_library_th.Export = "" Then %>
<% Breadcrumb.Render() %>
<% End If %>
<% e_library_th_delete.ShowPageHeader() %>
<% e_library_th_delete.ShowMessage %>
<form name="fe_library_thdelete" id="fe_library_thdelete" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="e_library_th">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<% For i = 0 to UBound(e_library_th_delete.RecKeys) %>
<input type="hidden" name="key_m" id="key_m" value="<%= ew_HtmlEncode(ew_GetKeyValue(e_library_th_delete.RecKeys(i))) %>">
<% Next %>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_e_library_thdelete" class="ewTable ewTableSeparate">
<%= e_library_th.TableCustomInnerHtml %>
	<thead>
	<tr class="ewTableHeader">
<% If e_library_th.el_id.Visible Then ' el_id %>
		<td><span id="elh_e_library_th_el_id" class="e_library_th_el_id"><%= e_library_th.el_id.FldCaption %></span></td>
<% End If %>
<% If e_library_th.el_date.Visible Then ' el_date %>
		<td><span id="elh_e_library_th_el_date" class="e_library_th_el_date"><%= e_library_th.el_date.FldCaption %></span></td>
<% End If %>
<% If e_library_th.el_title.Visible Then ' el_title %>
		<td><span id="elh_e_library_th_el_title" class="e_library_th_el_title"><%= e_library_th.el_title.FldCaption %></span></td>
<% End If %>
<% If e_library_th.el_pdf.Visible Then ' el_pdf %>
		<td><span id="elh_e_library_th_el_pdf" class="e_library_th_el_pdf"><%= e_library_th.el_pdf.FldCaption %></span></td>
<% End If %>
<% If e_library_th.el_img.Visible Then ' el_img %>
		<td><span id="elh_e_library_th_el_img" class="e_library_th_el_img"><%= e_library_th.el_img.FldCaption %></span></td>
<% End If %>
<% If e_library_th.el_create.Visible Then ' el_create %>
		<td><span id="elh_e_library_th_el_create" class="e_library_th_el_create"><%= e_library_th.el_create.FldCaption %></span></td>
<% End If %>
<% If e_library_th.el_update.Visible Then ' el_update %>
		<td><span id="elh_e_library_th_el_update" class="e_library_th_el_update"><%= e_library_th.el_update.FldCaption %></span></td>
<% End If %>
	</tr>
	</thead>
	<tbody>
<%
e_library_th_delete.RecCnt = 0
e_library_th_delete.RowCnt = 0
Do While (Not e_library_th_delete.Recordset.Eof)
	e_library_th_delete.RecCnt = e_library_th_delete.RecCnt + 1
	e_library_th_delete.RowCnt = e_library_th_delete.RowCnt + 1

	' Set row properties
	Call e_library_th.ResetAttrs()
	e_library_th.RowType = EW_ROWTYPE_VIEW ' view

	' Get the field contents
	Call e_library_th_delete.LoadRowValues(e_library_th_delete.Recordset)

	' Render row
	Call e_library_th_delete.RenderRow()
%>
	<tr<%= e_library_th.RowAttributes %>>
<% If e_library_th.el_id.Visible Then ' el_id %>
		<td<%= e_library_th.el_id.CellAttributes %>>
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_id" class="control-group e_library_th_el_id">
...
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_date" class="control-group e_library_th_el_date">
...
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_title" class="control-group e_library_th_el_title">
...
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_pdf" class="control-group e_library_th_el_pdf">
...
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_img" class="control-group e_library_th_el_img">
...
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_create" class="control-group e_library_th_el_create">
...
<span id="el<%= e_library_th_delete.RowCnt %>_e_library_th_el_update" class="control-group e_library_th_el_update">
...
<%
	e_library_th_delete.Recordset.MoveNext
Loop
e_library_th_delete.Recordset.Close
Set e_library_th_delete.Recordset = Nothing
%>
...
<script type="text/javascript">
fe_library_thdelete.Init();
</script>
<%
e_library_th_delete.ShowPageFooter()
...
' Drop page object
Set e_library_th_delete = Nothing
%>
<%
...
Class ce_library_th_delete
	...
	Public Property Get PageObjName()
		PageObjName = "e_library_th_delete"
	End Property
	...
	Sub Page_Main()
		...
		' Load Key Parameters
		RecKeys = e_library_th.GetRecordKeys() ' Load record keys
		sFilter = e_library_th.GetKeyFilter()
		If sFilter = "" Then
			Call Page_Terminate("pom_e_library_thlist.asp") ' Prevent SQL injection, return to list
		End If

		' Set up filter (Sql Where Clause) and get Return Sql
		' Sql constructor in e_library_th class, e_library_thinfo.asp

		e_library_th.CurrentFilter = sFilter

		' Get action
		If Request.Form("a_delete").Count > 0 Then
			e_library_th.CurrentAction = Request.Form("a_delete")
		Else
			e_library_th.CurrentAction = "I"	' Display record
		End If
		Select Case e_library_th.CurrentAction
			Case "D" ' Delete
				e_library_th.SendEmail = True ' Send email on delete success
				If DeleteRows() Then ' Delete rows
					SuccessMessage = Language.Phrase("DeleteSuccess") ' Set up success message
					Call Page_Terminate(e_library_th.ReturnUrl) ' Return to caller
				End If
		End Select
	End Sub
	...
	Function DeleteRows()
		...
		If DeleteRows Then
			...
			Do While Not RsDelete.Eof
				DeleteRows = e_library_th.Row_Deleting(RsDelete)
		...
		If DeleteRows Then
			...
			Do While Not RsOld.Eof
				Call e_library_th.Row_Deleted(RsOld)
				RsOld.MoveNext
			Loop
		End If
		RsOld.Close
