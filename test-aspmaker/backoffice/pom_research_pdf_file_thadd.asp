...
' Define page object
Dim research_pdf_file_th_add
Set research_pdf_file_th_add = New cresearch_pdf_file_th_add
Set Page = research_pdf_file_th_add

' Page init processing
research_pdf_file_th_add.Page_Init()

' Page main processing
research_pdf_file_th_add.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
research_pdf_file_th_add.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var research_pdf_file_th_add = new ew_Page("research_pdf_file_th_add");
research_pdf_file_th_add.PageID = "add"; // Page ID
var EW_PAGE_ID = research_pdf_file_th_add.PageID; // For backward compatibility
// Form object
var fresearch_pdf_file_thadd = new ew_Form("fresearch_pdf_file_thadd");
// Validate form
fresearch_pdf_file_thadd.Validate = function() {
...
fresearch_pdf_file_thadd.Form_CustomValidate = 
...
fresearch_pdf_file_thadd.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fresearch_pdf_file_thadd.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% Breadcrumb.Render() %>
<% End If %>
<% research_pdf_file_th_add.ShowPageHeader() %>
<% research_pdf_file_th_add.ShowMessage %>
<form name="fresearch_pdf_file_thadd" id="fresearch_pdf_file_thadd" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="research_pdf_file_th">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_research_pdf_file_thadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fresearch_pdf_file_thadd.Init();
<% If EW_MOBILE_REFLOW And ew_IsMobile() Then %>
ew_Reflow();
<% End If %>
</script>
<%
research_pdf_file_th_add.ShowPageFooter()
...
' Drop page object
Set research_pdf_file_th_add = Nothing
%>
<%
...
' Page Class
'
Class cresearch_pdf_file_th_add
	...
	Public Property Get PageObjName()
		PageObjName = "research_pdf_file_th_add"
	End Property
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			research_pdf_file_th.CurrentAction = ObjForm.GetValue("a_add") ' Get form action
		...
		Else
			...
			If Request.QueryString("rsh_pdf_id").Count > 0 Then
				...
				Call research_pdf_file_th.SetKey("rsh_pdf_id", research_pdf_file_th.rsh_pdf_id.CurrentValue) ' Set up key
			Else
				Call research_pdf_file_th.SetKey("rsh_pdf_id", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
				research_pdf_file_th.CurrentAction = "C" ' Copy Record
			Else
				research_pdf_file_th.CurrentAction = "I" ' Display Blank Record
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			If Not ValidateForm() Then
				research_pdf_file_th.CurrentAction = "I" ' Form error, reset action
		...
		Select Case research_pdf_file_th.CurrentAction
			...
			Case "A" ' Add new record
				research_pdf_file_th.SendEmail = True ' Send email on add success
				If AddRow(OldRecordset) Then ' Add successful
					SuccessMessage = Language.Phrase("AddSuccess") ' Set up success message
					Dim sReturnUrl
					sReturnUrl = research_pdf_file_th.ReturnUrl
					If ew_GetPageName(sReturnUrl) = "pom_research_pdf_file_thview.asp" Then sReturnUrl = research_pdf_file_th.ViewUrl("") ' View paging, return to view page with keyurl directly
		...
		research_pdf_file_th.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		research_pdf_file_th.rsh_id.CurrentValue = Null
		research_pdf_file_th.rsh_id.OldValue = research_pdf_file_th.rsh_id.CurrentValue
		research_pdf_file_th.rsh_pdf_file.CurrentValue = Null
		research_pdf_file_th.rsh_pdf_file.OldValue = research_pdf_file_th.rsh_pdf_file.CurrentValue
		research_pdf_file_th.rsh_pdf_title.CurrentValue = Null
		research_pdf_file_th.rsh_pdf_title.OldValue = research_pdf_file_th.rsh_pdf_title.CurrentValue
	End Function
	...
	Sub RenderRow()
		...
		ElseIf research_pdf_file_th.RowType = EW_ROWTYPE_ADD Then ' Add row
	...
	Function AddRow(RsOld)
		...
		' Field rsh_id
		Call research_pdf_file_th.rsh_id.SetDbValue(Rs, research_pdf_file_th.rsh_id.CurrentValue, Null, False)

		' Field rsh_pdf_file
		Call research_pdf_file_th.rsh_pdf_file.SetDbValue(Rs, research_pdf_file_th.rsh_pdf_file.CurrentValue, Null, False)

		' Field rsh_pdf_title
		Call research_pdf_file_th.rsh_pdf_title.SetDbValue(Rs, research_pdf_file_th.rsh_pdf_title.CurrentValue, Null, False)
		...
		bInsertRow = research_pdf_file_th.Row_Inserting(RsOld, Rs)
		If bInsertRow Then
		...
		Set Rs = Nothing
		If AddRow Then
			research_pdf_file_th.rsh_pdf_id.DbValue = RsNew("rsh_pdf_id")
		End If
		If AddRow Then

			' Call Row Inserted event
			Call research_pdf_file_th.Row_Inserted(RsOld, RsNew)
		End If
	...
	Sub SetupBreadcrumb()
		...
		PageId = ew_IIf(research_pdf_file_th.CurrentAction = "C", "Copy", "Add")
