...
' ASPMaker configuration for Table research_pdf_file_th
Dim research_pdf_file_th

' Define table class
Class cresearch_pdf_file_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "rsh_pdf_id", rsh_pdf_id)
		Call ew_SetArObj(Fields, "rsh_id", rsh_id)
		Call ew_SetArObj(Fields, "rsh_pdf_file", rsh_pdf_file)
		Call ew_SetArObj(Fields, "rsh_pdf_title", rsh_pdf_title)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "research_pdf_file_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [research_pdf_file_th]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[rsh_pdf_id] = @rsh_pdf_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(rsh_pdf_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@rsh_pdf_id@", ew_AdjustSql(rsh_pdf_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_research_pdf_file_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_research_pdf_file_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_research_pdf_file_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_research_pdf_file_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_research_pdf_file_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_research_pdf_file_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_research_pdf_file_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(rsh_pdf_id.CurrentValue) Then
			sUrl = sUrl & "rsh_pdf_id=" & rsh_pdf_id.CurrentValue
		Else
	...
	Function UrlParm(parm)
		If UseTokenInUrl Then
			UrlParm = "t=research_pdf_file_th"
		Else
			UrlParm = ""
		End If
		If parm <> "" Then
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("rsh_pdf_id") ' rsh_pdf_id
	...
	Public Sub LoadListRowValues(RsRow)
		rsh_pdf_id.DbValue = RsRow("rsh_pdf_id")
		rsh_id.DbValue = RsRow("rsh_id")
		rsh_pdf_file.DbValue = RsRow("rsh_pdf_file")
		rsh_pdf_title.DbValue = RsRow("rsh_pdf_title")
	End Sub
	...
	Sub RenderListRow()
		...
		rsh_pdf_id.ViewValue = rsh_pdf_id.CurrentValue
		rsh_pdf_id.ViewCustomAttributes = ""
		...
		' rsh_pdf_file
		rsh_pdf_file.ViewValue = rsh_pdf_file.CurrentValue
		rsh_pdf_file.ViewCustomAttributes = ""

		' rsh_pdf_title
		rsh_pdf_title.ViewValue = rsh_pdf_title.CurrentValue
		rsh_pdf_title.ViewCustomAttributes = ""

		' rsh_pdf_id
		rsh_pdf_id.LinkCustomAttributes = ""
		rsh_pdf_id.HrefValue = ""
		rsh_pdf_id.TooltipValue = ""
		...
		' rsh_pdf_file
		rsh_pdf_file.LinkCustomAttributes = ""
		rsh_pdf_file.HrefValue = ""
		rsh_pdf_file.TooltipValue = ""

		' rsh_pdf_title
		rsh_pdf_title.LinkCustomAttributes = ""
		rsh_pdf_title.HrefValue = ""
		rsh_pdf_title.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [research_pdf_file_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("rsh_pdf_id", rsh_pdf_id.ExportValue(Export))
					Call XmlDoc.AddField("rsh_id", rsh_id.ExportValue(Export))
					Call XmlDoc.AddField("rsh_pdf_file", rsh_pdf_file.ExportValue(Export))
					Call XmlDoc.AddField("rsh_pdf_title", rsh_pdf_title.ExportValue(Export))
				Else
					Call XmlDoc.AddField("rsh_pdf_id", rsh_pdf_id.ExportValue(Export))
					Call XmlDoc.AddField("rsh_id", rsh_id.ExportValue(Export))
					Call XmlDoc.AddField("rsh_pdf_file", rsh_pdf_file.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If rsh_pdf_id.Exportable Then Call Doc.ExportCaption(rsh_pdf_id)
				If rsh_id.Exportable Then Call Doc.ExportCaption(rsh_id)
				If rsh_pdf_file.Exportable Then Call Doc.ExportCaption(rsh_pdf_file)
				If rsh_pdf_title.Exportable Then Call Doc.ExportCaption(rsh_pdf_title)
			Else
				If rsh_pdf_id.Exportable Then Call Doc.ExportCaption(rsh_pdf_id)
				If rsh_id.Exportable Then Call Doc.ExportCaption(rsh_id)
				If rsh_pdf_file.Exportable Then Call Doc.ExportCaption(rsh_pdf_file)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If rsh_pdf_id.Exportable Then Call Doc.ExportField(rsh_pdf_id)
					If rsh_id.Exportable Then Call Doc.ExportField(rsh_id)
					If rsh_pdf_file.Exportable Then Call Doc.ExportField(rsh_pdf_file)
					If rsh_pdf_title.Exportable Then Call Doc.ExportField(rsh_pdf_title)
				Else
					If rsh_pdf_id.Exportable Then Call Doc.ExportField(rsh_pdf_id)
					If rsh_id.Exportable Then Call Doc.ExportField(rsh_id)
					If rsh_pdf_file.Exportable Then Call Doc.ExportField(rsh_pdf_file)
...
'			End If
	...
	' ----------------
	' Field rsh_pdf_id
	Private m_rsh_pdf_id

	Public Property Get rsh_pdf_id()
		If Not IsObject(m_rsh_pdf_id) Then
			Set m_rsh_pdf_id = NewFldObj("research_pdf_file_th", "research_pdf_file_th", "x_rsh_pdf_id", "rsh_pdf_id", "[rsh_pdf_id]", "[rsh_pdf_id]", 3, 0, "[rsh_pdf_id]", False, False, FALSE, "FORMATTED TEXT")
			m_rsh_pdf_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set rsh_pdf_id = m_rsh_pdf_id
	End Property
	...
	Public Property Get rsh_id()
		If Not IsObject(m_rsh_id) Then
			Set m_rsh_id = NewFldObj("research_pdf_file_th", "research_pdf_file_th", "x_rsh_id", "rsh_id", "[rsh_id]", "[rsh_id]", 3, 0, "[rsh_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	' Field rsh_pdf_file
	Private m_rsh_pdf_file

	Public Property Get rsh_pdf_file()
		If Not IsObject(m_rsh_pdf_file) Then
			Set m_rsh_pdf_file = NewFldObj("research_pdf_file_th", "research_pdf_file_th", "x_rsh_pdf_file", "rsh_pdf_file", "[rsh_pdf_file]", "[rsh_pdf_file]", 202, 0, "[rsh_pdf_file]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set rsh_pdf_file = m_rsh_pdf_file
	End Property

	' Field rsh_pdf_title
	Private m_rsh_pdf_title

	Public Property Get rsh_pdf_title()
		If Not IsObject(m_rsh_pdf_title) Then
			Set m_rsh_pdf_title = NewFldObj("research_pdf_file_th", "research_pdf_file_th", "x_rsh_pdf_title", "rsh_pdf_title", "[rsh_pdf_title]", "[rsh_pdf_title]", 202, 0, "[rsh_pdf_title]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set rsh_pdf_title = m_rsh_pdf_title
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_rsh_pdf_id) Then Set m_rsh_pdf_id = Nothing
		If IsObject(m_rsh_id) Then Set m_rsh_id = Nothing
		If IsObject(m_rsh_pdf_file) Then Set m_rsh_pdf_file = Nothing
		If IsObject(m_rsh_pdf_title) Then Set m_rsh_pdf_title = Nothing
