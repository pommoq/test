...
' Define page object
Dim department_th_add
Set department_th_add = New cdepartment_th_add
Set Page = department_th_add

' Page init processing
department_th_add.Page_Init()

' Page main processing
department_th_add.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
department_th_add.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var department_th_add = new ew_Page("department_th_add");
department_th_add.PageID = "add"; // Page ID
var EW_PAGE_ID = department_th_add.PageID; // For backward compatibility
// Form object
var fdepartment_thadd = new ew_Form("fdepartment_thadd");
// Validate form
fdepartment_thadd.Validate = function() {
...
fdepartment_thadd.Form_CustomValidate = 
...
fdepartment_thadd.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fdepartment_thadd.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% Breadcrumb.Render() %>
<% End If %>
<% department_th_add.ShowPageHeader() %>
<% department_th_add.ShowMessage %>
<form name="fdepartment_thadd" id="fdepartment_thadd" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="department_th">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_department_thadd" class="table table-bordered table-striped">
...
<input type="text" data-field="x_dept_id" name="x_dept_id" id="x_dept_id" size="30" placeholder="<%= department_th.dept_id.PlaceHolder %>" value="<%= department_th.dept_id.EditValue %>"<%= department_th.dept_id.EditAttributes %>>
</span>
...
<script type="text/javascript">
fdepartment_thadd.Init();
<% If EW_MOBILE_REFLOW And ew_IsMobile() Then %>
ew_Reflow();
<% End If %>
</script>
<%
department_th_add.ShowPageFooter()
...
Class cdepartment_th_add
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			department_th.CurrentAction = ObjForm.GetValue("a_add") ' Get form action
		...
		Else
			...
			If Request.QueryString("dept_id").Count > 0 Then
				...
				Call department_th.SetKey("dept_id", department_th.dept_id.CurrentValue) ' Set up key
			Else
				Call department_th.SetKey("dept_id", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
				department_th.CurrentAction = "C" ' Copy Record
			Else
				department_th.CurrentAction = "I" ' Display Blank Record
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			If Not ValidateForm() Then
				department_th.CurrentAction = "I" ' Form error, reset action
		...
		Select Case department_th.CurrentAction
			...
			Case "A" ' Add new record
				department_th.SendEmail = True ' Send email on add success
				If AddRow(OldRecordset) Then ' Add successful
					SuccessMessage = Language.Phrase("AddSuccess") ' Set up success message
					Dim sReturnUrl
					sReturnUrl = department_th.ReturnUrl
					If ew_GetPageName(sReturnUrl) = "pom_department_thview.asp" Then sReturnUrl = department_th.ViewUrl("") ' View paging, return to view page with keyurl directly
		...
		department_th.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		department_th.dept_id.CurrentValue = Null
		department_th.dept_id.OldValue = department_th.dept_id.CurrentValue
		department_th.office_id.CurrentValue = Null
		department_th.office_id.OldValue = department_th.office_id.CurrentValue
		department_th.dept_name.CurrentValue = Null
		department_th.dept_name.OldValue = department_th.dept_name.CurrentValue
		department_th.dept_sort.CurrentValue = Null
		department_th.dept_sort.OldValue = department_th.dept_sort.CurrentValue
	End Function
	...
	Sub RenderRow()
		...
		ElseIf department_th.RowType = EW_ROWTYPE_ADD Then ' Add row

			' dept_id
			department_th.dept_id.EditCustomAttributes = ""
			department_th.dept_id.EditValue = ew_HtmlEncode(department_th.dept_id.CurrentValue)
			department_th.dept_id.PlaceHolder = ew_HtmlEncode(ew_RemoveHtml(department_th.dept_id.FldCaption))
	...
	Function AddRow(RsOld)
		...
		If department_th.dept_id.CurrentValue <> "" Then ' Check field with unique index
			sFilter = "([dept_id] = " & ew_AdjustSql(department_th.dept_id.CurrentValue) & ")"
			Set RsChk = department_th.LoadRs(sFilter)
			If Not (RsChk Is Nothing) Then
				sIdxErrMsg = Replace(Language.Phrase("DupIndex"), "%f", department_th.dept_id.FldCaption)
				sIdxErrMsg = Replace(sIdxErrMsg, "%v", department_th.dept_id.CurrentValue)
		...
		' Field dept_id
		Call department_th.dept_id.SetDbValue(Rs, department_th.dept_id.CurrentValue, Null, False)

		' Field office_id
		Call department_th.office_id.SetDbValue(Rs, department_th.office_id.CurrentValue, Null, False)

		' Field dept_name
		Call department_th.dept_name.SetDbValue(Rs, department_th.dept_name.CurrentValue, Null, False)

		' Field dept_sort
		Call department_th.dept_sort.SetDbValue(Rs, department_th.dept_sort.CurrentValue, Null, False)
		...
		bInsertRow = department_th.Row_Inserting(RsOld, Rs)

		' Check if key value entered
		If bInsertRow And department_th.ValidateKey And department_th.dept_id.CurrentValue = "" And department_th.dept_id.SessionValue = "" Then
		...
		Dim sKeyErrMsg
		If bInsertRow And department_th.ValidateKey Then
			sFilter = department_th.KeyFilter
			Set RsChk = department_th.LoadRs(sFilter)
			If Not (RsChk Is Nothing) Then
		...
		If AddRow Then
			...
			Call department_th.Row_Inserted(RsOld, RsNew)
		End If
	...
	Sub SetupBreadcrumb()
		...
		PageId = ew_IIf(department_th.CurrentAction = "C", "Copy", "Add")
