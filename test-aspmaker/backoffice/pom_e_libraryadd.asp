...
' Define page object
Dim e_library_add
Set e_library_add = New ce_library_add
Set Page = e_library_add

' Page init processing
e_library_add.Page_Init()

' Page main processing
e_library_add.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
e_library_add.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var e_library_add = new ew_Page("e_library_add");
e_library_add.PageID = "add"; // Page ID
var EW_PAGE_ID = e_library_add.PageID; // For backward compatibility
// Form object
var fe_libraryadd = new ew_Form("fe_libraryadd");
// Validate form
fe_libraryadd.Validate = function() {
...
fe_libraryadd.Form_CustomValidate = 
...
fe_libraryadd.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fe_libraryadd.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% Breadcrumb.Render() %>
<% End If %>
<% e_library_add.ShowPageHeader() %>
<% e_library_add.ShowMessage %>
<form name="fe_libraryadd" id="fe_libraryadd" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="e_library">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_e_libraryadd" class="table table-bordered table-striped">
...
<input type="text" data-field="x_el_id" name="x_el_id" id="x_el_id" size="30" placeholder="<%= e_library.el_id.PlaceHolder %>" value="<%= e_library.el_id.EditValue %>"<%= e_library.el_id.EditAttributes %>>
</span>
...
Class ce_library_add
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			e_library.CurrentAction = ObjForm.GetValue("a_add") ' Get form action
		...
		Else
			...
			If Request.QueryString("el_id").Count > 0 Then
				...
				Call e_library.SetKey("el_id", e_library.el_id.CurrentValue) ' Set up key
			Else
				Call e_library.SetKey("el_id", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
				e_library.CurrentAction = "C" ' Copy Record
			Else
				e_library.CurrentAction = "I" ' Display Blank Record
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			If Not ValidateForm() Then
				e_library.CurrentAction = "I" ' Form error, reset action
		...
		Select Case e_library.CurrentAction
			...
			Case "A" ' Add new record
				e_library.SendEmail = True ' Send email on add success
				If AddRow(OldRecordset) Then ' Add successful
					SuccessMessage = Language.Phrase("AddSuccess") ' Set up success message
					Dim sReturnUrl
					sReturnUrl = e_library.ReturnUrl
					If ew_GetPageName(sReturnUrl) = "pom_e_libraryview.asp" Then sReturnUrl = e_library.ViewUrl("") ' View paging, return to view page with keyurl directly
		...
		e_library.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		e_library.el_id.CurrentValue = Null
		e_library.el_id.OldValue = e_library.el_id.CurrentValue
		e_library.el_date.CurrentValue = Null
		e_library.el_date.OldValue = e_library.el_date.CurrentValue
		e_library.el_title.CurrentValue = Null
		e_library.el_title.OldValue = e_library.el_title.CurrentValue
		e_library.el_pdf.CurrentValue = Null
		e_library.el_pdf.OldValue = e_library.el_pdf.CurrentValue
		e_library.el_img.CurrentValue = Null
		e_library.el_img.OldValue = e_library.el_img.CurrentValue
		e_library.el_detail.CurrentValue = Null
		e_library.el_detail.OldValue = e_library.el_detail.CurrentValue
		e_library.el_create.CurrentValue = Null
		e_library.el_create.OldValue = e_library.el_create.CurrentValue
		e_library.el_update.CurrentValue = Null
		e_library.el_update.OldValue = e_library.el_update.CurrentValue
	End Function
	...
	Sub RenderRow()
		...
		ElseIf e_library.RowType = EW_ROWTYPE_ADD Then ' Add row

			' el_id
			e_library.el_id.EditCustomAttributes = ""
			e_library.el_id.EditValue = ew_HtmlEncode(e_library.el_id.CurrentValue)
			e_library.el_id.PlaceHolder = ew_HtmlEncode(ew_RemoveHtml(e_library.el_id.FldCaption))
	...
	Function AddRow(RsOld)
		...
		If e_library.el_id.CurrentValue <> "" Then ' Check field with unique index
			sFilter = "([el_id] = " & ew_AdjustSql(e_library.el_id.CurrentValue) & ")"
			Set RsChk = e_library.LoadRs(sFilter)
			If Not (RsChk Is Nothing) Then
				sIdxErrMsg = Replace(Language.Phrase("DupIndex"), "%f", e_library.el_id.FldCaption)
				sIdxErrMsg = Replace(sIdxErrMsg, "%v", e_library.el_id.CurrentValue)
		...
		' Field el_id
		Call e_library.el_id.SetDbValue(Rs, e_library.el_id.CurrentValue, Null, False)

		' Field el_date
		Call e_library.el_date.SetDbValue(Rs, e_library.el_date.CurrentValue, Null, False)

		' Field el_title
		Call e_library.el_title.SetDbValue(Rs, e_library.el_title.CurrentValue, Null, False)

		' Field el_pdf
		Call e_library.el_pdf.SetDbValue(Rs, e_library.el_pdf.CurrentValue, Null, False)

		' Field el_img
		Call e_library.el_img.SetDbValue(Rs, e_library.el_img.CurrentValue, Null, False)

		' Field el_detail
		Call e_library.el_detail.SetDbValue(Rs, e_library.el_detail.CurrentValue, Null, False)

		' Field el_create
		Call e_library.el_create.SetDbValue(Rs, e_library.el_create.CurrentValue, Null, False)

		' Field el_update
		Call e_library.el_update.SetDbValue(Rs, e_library.el_update.CurrentValue, Null, False)
		...
		bInsertRow = e_library.Row_Inserting(RsOld, Rs)

		' Check if key value entered
		If bInsertRow And e_library.ValidateKey And e_library.el_id.CurrentValue = "" And e_library.el_id.SessionValue = "" Then
		...
		Dim sKeyErrMsg
		If bInsertRow And e_library.ValidateKey Then
			sFilter = e_library.KeyFilter
			Set RsChk = e_library.LoadRs(sFilter)
			If Not (RsChk Is Nothing) Then
		...
		If AddRow Then
			...
			Call e_library.Row_Inserted(RsOld, RsNew)
		End If
	...
	Sub SetupBreadcrumb()
		...
		PageId = ew_IIf(e_library.CurrentAction = "C", "Copy", "Add")
