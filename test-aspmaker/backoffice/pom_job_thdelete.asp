...
' Define page object
Dim job_th_delete
Set job_th_delete = New cjob_th_delete
Set Page = job_th_delete

' Page init processing
job_th_delete.Page_Init()

' Page main processing
job_th_delete.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
job_th_delete.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var job_th_delete = new ew_Page("job_th_delete");
job_th_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = job_th_delete.PageID; // For backward compatibility
// Form object
var fjob_thdelete = new ew_Form("fjob_thdelete");
// Form_CustomValidate event
fjob_thdelete.Form_CustomValidate = 
...
fjob_thdelete.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fjob_thdelete.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
Set job_th_delete.Recordset = job_th_delete.LoadRecordset()
job_th_delete.TotalRecs = job_th_delete.Recordset.RecordCount ' Get record count
If job_th_delete.TotalRecs <= 0 Then ' No record found, exit
	job_th_delete.Recordset.Close
	Set job_th_delete.Recordset = Nothing
	Call job_th_delete.Page_Terminate("pom_job_thlist.asp") ' Return to list
End If
%>
<% If job_th.Export = "" Then %>
<% Breadcrumb.Render() %>
<% End If %>
<% job_th_delete.ShowPageHeader() %>
<% job_th_delete.ShowMessage %>
<form name="fjob_thdelete" id="fjob_thdelete" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="job_th">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<% For i = 0 to UBound(job_th_delete.RecKeys) %>
<input type="hidden" name="key_m" id="key_m" value="<%= ew_HtmlEncode(ew_GetKeyValue(job_th_delete.RecKeys(i))) %>">
<% Next %>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_job_thdelete" class="ewTable ewTableSeparate">
<%= job_th.TableCustomInnerHtml %>
	<thead>
	<tr class="ewTableHeader">
<% If job_th.job_id.Visible Then ' job_id %>
		<td><span id="elh_job_th_job_id" class="job_th_job_id"><%= job_th.job_id.FldCaption %></span></td>
<% End If %>
<% If job_th.company_id.Visible Then ' company_id %>
		<td><span id="elh_job_th_company_id" class="job_th_company_id"><%= job_th.company_id.FldCaption %></span></td>
<% End If %>
<% If job_th.job_date.Visible Then ' job_date %>
		<td><span id="elh_job_th_job_date" class="job_th_job_date"><%= job_th.job_date.FldCaption %></span></td>
<% End If %>
<% If job_th.job_title.Visible Then ' job_title %>
		<td><span id="elh_job_th_job_title" class="job_th_job_title"><%= job_th.job_title.FldCaption %></span></td>
<% End If %>
<% If job_th.job_create.Visible Then ' job_create %>
		<td><span id="elh_job_th_job_create" class="job_th_job_create"><%= job_th.job_create.FldCaption %></span></td>
<% End If %>
<% If job_th.job_update.Visible Then ' job_update %>
		<td><span id="elh_job_th_job_update" class="job_th_job_update"><%= job_th.job_update.FldCaption %></span></td>
<% End If %>
<% If job_th.job_show.Visible Then ' job_show %>
		<td><span id="elh_job_th_job_show" class="job_th_job_show"><%= job_th.job_show.FldCaption %></span></td>
<% End If %>
	</tr>
	</thead>
	<tbody>
<%
job_th_delete.RecCnt = 0
job_th_delete.RowCnt = 0
Do While (Not job_th_delete.Recordset.Eof)
	job_th_delete.RecCnt = job_th_delete.RecCnt + 1
	job_th_delete.RowCnt = job_th_delete.RowCnt + 1

	' Set row properties
	Call job_th.ResetAttrs()
	job_th.RowType = EW_ROWTYPE_VIEW ' view

	' Get the field contents
	Call job_th_delete.LoadRowValues(job_th_delete.Recordset)

	' Render row
	Call job_th_delete.RenderRow()
%>
	<tr<%= job_th.RowAttributes %>>
<% If job_th.job_id.Visible Then ' job_id %>
		<td<%= job_th.job_id.CellAttributes %>>
<span id="el<%= job_th_delete.RowCnt %>_job_th_job_id" class="control-group job_th_job_id">
...
<span id="el<%= job_th_delete.RowCnt %>_job_th_company_id" class="control-group job_th_company_id">
...
<span id="el<%= job_th_delete.RowCnt %>_job_th_job_date" class="control-group job_th_job_date">
...
<span id="el<%= job_th_delete.RowCnt %>_job_th_job_title" class="control-group job_th_job_title">
...
<span id="el<%= job_th_delete.RowCnt %>_job_th_job_create" class="control-group job_th_job_create">
...
<span id="el<%= job_th_delete.RowCnt %>_job_th_job_update" class="control-group job_th_job_update">
...
<span id="el<%= job_th_delete.RowCnt %>_job_th_job_show" class="control-group job_th_job_show">
...
<%
	job_th_delete.Recordset.MoveNext
Loop
job_th_delete.Recordset.Close
Set job_th_delete.Recordset = Nothing
%>
...
Class cjob_th_delete
	...
	Sub Page_Main()
		...
		' Load Key Parameters
		RecKeys = job_th.GetRecordKeys() ' Load record keys
		sFilter = job_th.GetKeyFilter()
		If sFilter = "" Then
			Call Page_Terminate("pom_job_thlist.asp") ' Prevent SQL injection, return to list
		End If

		' Set up filter (Sql Where Clause) and get Return Sql
		' Sql constructor in job_th class, job_thinfo.asp

		job_th.CurrentFilter = sFilter

		' Get action
		If Request.Form("a_delete").Count > 0 Then
			job_th.CurrentAction = Request.Form("a_delete")
		Else
			job_th.CurrentAction = "I"	' Display record
		End If
		Select Case job_th.CurrentAction
			Case "D" ' Delete
				job_th.SendEmail = True ' Send email on delete success
				If DeleteRows() Then ' Delete rows
					SuccessMessage = Language.Phrase("DeleteSuccess") ' Set up success message
					Call Page_Terminate(job_th.ReturnUrl) ' Return to caller
				End If
		End Select
	End Sub
	...
	Function DeleteRows()
		...
		If DeleteRows Then
			...
			Do While Not RsDelete.Eof
				DeleteRows = job_th.Row_Deleting(RsDelete)
