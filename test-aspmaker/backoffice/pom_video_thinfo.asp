...
' ASPMaker configuration for Table video_th
Dim video_th
...
Class cvideo_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "video_id", video_id)
		Call ew_SetArObj(Fields, "video_title", video_title)
		Call ew_SetArObj(Fields, "video_link", video_link)
		Call ew_SetArObj(Fields, "video_detail", video_detail)
		Call ew_SetArObj(Fields, "video_create", video_create)
		Call ew_SetArObj(Fields, "video_update", video_update)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "video_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [video_th]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[video_id] = @video_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(video_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@video_id@", ew_AdjustSql(video_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_video_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_video_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_video_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_video_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_video_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_video_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_video_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(video_id.CurrentValue) Then
			sUrl = sUrl & "video_id=" & video_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("video_id") ' video_id
	...
	Public Sub LoadListRowValues(RsRow)
		video_id.DbValue = RsRow("video_id")
		video_title.DbValue = RsRow("video_title")
		video_link.DbValue = RsRow("video_link")
		video_detail.DbValue = RsRow("video_detail")
		video_create.DbValue = RsRow("video_create")
		video_update.DbValue = RsRow("video_update")
	End Sub
	...
	Sub RenderListRow()
		...
		video_id.ViewValue = video_id.CurrentValue
		video_id.ViewCustomAttributes = ""

		' video_title
		video_title.ViewValue = video_title.CurrentValue
		video_title.ViewCustomAttributes = ""

		' video_link
		video_link.ViewValue = video_link.CurrentValue
		video_link.ViewCustomAttributes = ""

		' video_detail
		video_detail.ViewValue = video_detail.CurrentValue
		video_detail.ViewCustomAttributes = ""

		' video_create
		video_create.ViewValue = video_create.CurrentValue
		video_create.ViewCustomAttributes = ""

		' video_update
		video_update.ViewValue = video_update.CurrentValue
		video_update.ViewCustomAttributes = ""

		' video_id
		video_id.LinkCustomAttributes = ""
		video_id.HrefValue = ""
		video_id.TooltipValue = ""

		' video_title
		video_title.LinkCustomAttributes = ""
		video_title.HrefValue = ""
		video_title.TooltipValue = ""

		' video_link
		video_link.LinkCustomAttributes = ""
		video_link.HrefValue = ""
		video_link.TooltipValue = ""

		' video_detail
		video_detail.LinkCustomAttributes = ""
		video_detail.HrefValue = ""
		video_detail.TooltipValue = ""

		' video_create
		video_create.LinkCustomAttributes = ""
		video_create.HrefValue = ""
		video_create.TooltipValue = ""

		' video_update
		video_update.LinkCustomAttributes = ""
		video_update.HrefValue = ""
		video_update.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [video_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("video_id", video_id.ExportValue(Export))
					Call XmlDoc.AddField("video_title", video_title.ExportValue(Export))
					Call XmlDoc.AddField("video_link", video_link.ExportValue(Export))
					Call XmlDoc.AddField("video_detail", video_detail.ExportValue(Export))
					Call XmlDoc.AddField("video_create", video_create.ExportValue(Export))
					Call XmlDoc.AddField("video_update", video_update.ExportValue(Export))
				Else
					Call XmlDoc.AddField("video_id", video_id.ExportValue(Export))
					Call XmlDoc.AddField("video_title", video_title.ExportValue(Export))
					Call XmlDoc.AddField("video_link", video_link.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If video_id.Exportable Then Call Doc.ExportCaption(video_id)
				If video_title.Exportable Then Call Doc.ExportCaption(video_title)
				If video_link.Exportable Then Call Doc.ExportCaption(video_link)
				If video_detail.Exportable Then Call Doc.ExportCaption(video_detail)
				If video_create.Exportable Then Call Doc.ExportCaption(video_create)
				If video_update.Exportable Then Call Doc.ExportCaption(video_update)
			Else
				If video_id.Exportable Then Call Doc.ExportCaption(video_id)
				If video_title.Exportable Then Call Doc.ExportCaption(video_title)
				If video_link.Exportable Then Call Doc.ExportCaption(video_link)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If video_id.Exportable Then Call Doc.ExportField(video_id)
					If video_title.Exportable Then Call Doc.ExportField(video_title)
					If video_link.Exportable Then Call Doc.ExportField(video_link)
					If video_detail.Exportable Then Call Doc.ExportField(video_detail)
					If video_create.Exportable Then Call Doc.ExportField(video_create)
					If video_update.Exportable Then Call Doc.ExportField(video_update)
				Else
					If video_id.Exportable Then Call Doc.ExportField(video_id)
					If video_title.Exportable Then Call Doc.ExportField(video_title)
					If video_link.Exportable Then Call Doc.ExportField(video_link)
...
'			End If
	...
	Public Property Get video_id()
		If Not IsObject(m_video_id) Then
			Set m_video_id = NewFldObj("video_th", "video_th", "x_video_id", "video_id", "[video_id]", "[video_id]", 3, 0, "[video_id]", False, False, FALSE, "FORMATTED TEXT")
			m_video_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set video_id = m_video_id
	End Property

	' Field video_title
	Private m_video_title

	Public Property Get video_title()
		If Not IsObject(m_video_title) Then
			Set m_video_title = NewFldObj("video_th", "video_th", "x_video_title", "video_title", "[video_title]", "[video_title]", 202, 0, "[video_title]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set video_title = m_video_title
	End Property

	' Field video_link
	Private m_video_link

	Public Property Get video_link()
		If Not IsObject(m_video_link) Then
			Set m_video_link = NewFldObj("video_th", "video_th", "x_video_link", "video_link", "[video_link]", "[video_link]", 202, 0, "[video_link]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set video_link = m_video_link
	End Property

	' Field video_detail
	Private m_video_detail

	Public Property Get video_detail()
		If Not IsObject(m_video_detail) Then
			Set m_video_detail = NewFldObj("video_th", "video_th", "x_video_detail", "video_detail", "[video_detail]", "[video_detail]", 203, 0, "[video_detail]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set video_detail = m_video_detail
	End Property

	' Field video_create
	Private m_video_create

	Public Property Get video_create()
		If Not IsObject(m_video_create) Then
			Set m_video_create = NewFldObj("video_th", "video_th", "x_video_create", "video_create", "[video_create]", "FORMAT([video_create], '')", 135, 8, "[video_create]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set video_create = m_video_create
	End Property

	' Field video_update
	Private m_video_update

	Public Property Get video_update()
		If Not IsObject(m_video_update) Then
			Set m_video_update = NewFldObj("video_th", "video_th", "x_video_update", "video_update", "[video_update]", "FORMAT([video_update], '')", 135, 8, "[video_update]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set video_update = m_video_update
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_video_id) Then Set m_video_id = Nothing
		If IsObject(m_video_title) Then Set m_video_title = Nothing
		If IsObject(m_video_link) Then Set m_video_link = Nothing
		If IsObject(m_video_detail) Then Set m_video_detail = Nothing
		If IsObject(m_video_create) Then Set m_video_create = Nothing
		If IsObject(m_video_update) Then Set m_video_update = Nothing
