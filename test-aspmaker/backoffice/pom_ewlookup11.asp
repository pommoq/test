<%@ CodePage="65001" LCID="1054" EnableSessionState="False" %>
<%
...
Class clookup
	...
	Sub Page_Main()
		...
		End If
		sql = Request.Form("s") & ""
		sql = ew_Decrypt(ew_Decode(sql), EW_RANDOM_KEY)
		If InStr(sql, "{filter}") > 0 Then
			...
			For i = 0 to 4
				...
				filter = ew_Decrypt(filter, EW_RANDOM_KEY)
				If filter <> "" Then
		...
		value = ew_AdjustSql(Request.Form("q"))
		If value & "" <> "" Then
