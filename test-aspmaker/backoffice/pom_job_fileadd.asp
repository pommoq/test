...
' Define page object
Dim job_file_add
Set job_file_add = New cjob_file_add
Set Page = job_file_add

' Page init processing
job_file_add.Page_Init()

' Page main processing
job_file_add.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
job_file_add.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var job_file_add = new ew_Page("job_file_add");
job_file_add.PageID = "add"; // Page ID
var EW_PAGE_ID = job_file_add.PageID; // For backward compatibility
// Form object
var fjob_fileadd = new ew_Form("fjob_fileadd");
// Validate form
fjob_fileadd.Validate = function() {
...
fjob_fileadd.Form_CustomValidate = 
...
fjob_fileadd.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fjob_fileadd.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% Breadcrumb.Render() %>
<% End If %>
<% job_file_add.ShowPageHeader() %>
<% job_file_add.ShowMessage %>
<form name="fjob_fileadd" id="fjob_fileadd" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="job_file">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_job_fileadd" class="table table-bordered table-striped">
...
Class cjob_file_add
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			job_file.CurrentAction = ObjForm.GetValue("a_add") ' Get form action
		...
		Else
			...
			If Request.QueryString("job_file_id").Count > 0 Then
				...
				Call job_file.SetKey("job_file_id", job_file.job_file_id.CurrentValue) ' Set up key
			Else
				Call job_file.SetKey("job_file_id", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
				job_file.CurrentAction = "C" ' Copy Record
			Else
				job_file.CurrentAction = "I" ' Display Blank Record
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			If Not ValidateForm() Then
				job_file.CurrentAction = "I" ' Form error, reset action
		...
		Select Case job_file.CurrentAction
			...
			Case "A" ' Add new record
				job_file.SendEmail = True ' Send email on add success
				If AddRow(OldRecordset) Then ' Add successful
					SuccessMessage = Language.Phrase("AddSuccess") ' Set up success message
					Dim sReturnUrl
					sReturnUrl = job_file.ReturnUrl
					If ew_GetPageName(sReturnUrl) = "pom_job_fileview.asp" Then sReturnUrl = job_file.ViewUrl("") ' View paging, return to view page with keyurl directly
		...
		job_file.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		job_file.job_id.CurrentValue = Null
		job_file.job_id.OldValue = job_file.job_id.CurrentValue
		job_file.job_file_name.CurrentValue = Null
		job_file.job_file_name.OldValue = job_file.job_file_name.CurrentValue
		job_file.job_file_title.CurrentValue = Null
		job_file.job_file_title.OldValue = job_file.job_file_title.CurrentValue
	End Function
	...
	Sub RenderRow()
		...
		ElseIf job_file.RowType = EW_ROWTYPE_ADD Then ' Add row
	...
	Function AddRow(RsOld)
		...
		' Field job_id
		Call job_file.job_id.SetDbValue(Rs, job_file.job_id.CurrentValue, Null, False)

		' Field job_file_name
		Call job_file.job_file_name.SetDbValue(Rs, job_file.job_file_name.CurrentValue, Null, False)

		' Field job_file_title
		Call job_file.job_file_title.SetDbValue(Rs, job_file.job_file_title.CurrentValue, Null, False)
		...
		bInsertRow = job_file.Row_Inserting(RsOld, Rs)
		If bInsertRow Then
		...
		Set Rs = Nothing
		If AddRow Then
			job_file.job_file_id.DbValue = RsNew("job_file_id")
		End If
		If AddRow Then

			' Call Row Inserted event
			Call job_file.Row_Inserted(RsOld, RsNew)
		End If
	...
	Sub SetupBreadcrumb()
		...
		PageId = ew_IIf(job_file.CurrentAction = "C", "Copy", "Add")
