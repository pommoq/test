...
' Define page object
Dim sys_admin_menu_delete
Set sys_admin_menu_delete = New csys_admin_menu_delete
Set Page = sys_admin_menu_delete

' Page init processing
sys_admin_menu_delete.Page_Init()

' Page main processing
sys_admin_menu_delete.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
sys_admin_menu_delete.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var sys_admin_menu_delete = new ew_Page("sys_admin_menu_delete");
sys_admin_menu_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = sys_admin_menu_delete.PageID; // For backward compatibility
// Form object
var fsys_admin_menudelete = new ew_Form("fsys_admin_menudelete");
// Form_CustomValidate event
fsys_admin_menudelete.Form_CustomValidate = 
...
fsys_admin_menudelete.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fsys_admin_menudelete.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
Set sys_admin_menu_delete.Recordset = sys_admin_menu_delete.LoadRecordset()
sys_admin_menu_delete.TotalRecs = sys_admin_menu_delete.Recordset.RecordCount ' Get record count
If sys_admin_menu_delete.TotalRecs <= 0 Then ' No record found, exit
	sys_admin_menu_delete.Recordset.Close
	Set sys_admin_menu_delete.Recordset = Nothing
	Call sys_admin_menu_delete.Page_Terminate("pom_sys_admin_menulist.asp") ' Return to list
End If
%>
<% If sys_admin_menu.Export = "" Then %>
<% Breadcrumb.Render() %>
<% End If %>
<% sys_admin_menu_delete.ShowPageHeader() %>
<% sys_admin_menu_delete.ShowMessage %>
<form name="fsys_admin_menudelete" id="fsys_admin_menudelete" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="sys_admin_menu">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<% For i = 0 to UBound(sys_admin_menu_delete.RecKeys) %>
<input type="hidden" name="key_m" id="key_m" value="<%= ew_HtmlEncode(ew_GetKeyValue(sys_admin_menu_delete.RecKeys(i))) %>">
<% Next %>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_sys_admin_menudelete" class="ewTable ewTableSeparate">
<%= sys_admin_menu.TableCustomInnerHtml %>
	<thead>
	<tr class="ewTableHeader">
<% If sys_admin_menu.sys_admin_menu_id.Visible Then ' sys_admin_menu_id %>
		<td><span id="elh_sys_admin_menu_sys_admin_menu_id" class="sys_admin_menu_sys_admin_menu_id"><%= sys_admin_menu.sys_admin_menu_id.FldCaption %></span></td>
<% End If %>
<% If sys_admin_menu.admin_id.Visible Then ' admin_id %>
		<td><span id="elh_sys_admin_menu_admin_id" class="sys_admin_menu_admin_id"><%= sys_admin_menu.admin_id.FldCaption %></span></td>
<% End If %>
<% If sys_admin_menu.menu_id.Visible Then ' menu_id %>
		<td><span id="elh_sys_admin_menu_menu_id" class="sys_admin_menu_menu_id"><%= sys_admin_menu.menu_id.FldCaption %></span></td>
<% End If %>
	</tr>
	</thead>
	<tbody>
<%
sys_admin_menu_delete.RecCnt = 0
sys_admin_menu_delete.RowCnt = 0
Do While (Not sys_admin_menu_delete.Recordset.Eof)
	sys_admin_menu_delete.RecCnt = sys_admin_menu_delete.RecCnt + 1
	sys_admin_menu_delete.RowCnt = sys_admin_menu_delete.RowCnt + 1

	' Set row properties
	Call sys_admin_menu.ResetAttrs()
	sys_admin_menu.RowType = EW_ROWTYPE_VIEW ' view

	' Get the field contents
	Call sys_admin_menu_delete.LoadRowValues(sys_admin_menu_delete.Recordset)

	' Render row
	Call sys_admin_menu_delete.RenderRow()
%>
	<tr<%= sys_admin_menu.RowAttributes %>>
<% If sys_admin_menu.sys_admin_menu_id.Visible Then ' sys_admin_menu_id %>
		<td<%= sys_admin_menu.sys_admin_menu_id.CellAttributes %>>
<span id="el<%= sys_admin_menu_delete.RowCnt %>_sys_admin_menu_sys_admin_menu_id" class="control-group sys_admin_menu_sys_admin_menu_id">
...
<span id="el<%= sys_admin_menu_delete.RowCnt %>_sys_admin_menu_admin_id" class="control-group sys_admin_menu_admin_id">
...
<span id="el<%= sys_admin_menu_delete.RowCnt %>_sys_admin_menu_menu_id" class="control-group sys_admin_menu_menu_id">
...
<%
	sys_admin_menu_delete.Recordset.MoveNext
Loop
sys_admin_menu_delete.Recordset.Close
Set sys_admin_menu_delete.Recordset = Nothing
%>
...
<script type="text/javascript">
fsys_admin_menudelete.Init();
</script>
<%
sys_admin_menu_delete.ShowPageFooter()
...
' Drop page object
Set sys_admin_menu_delete = Nothing
%>
<%
...
Class csys_admin_menu_delete
	...
	Public Property Get PageObjName()
		PageObjName = "sys_admin_menu_delete"
	End Property
	...
	Sub Page_Main()
		...
		' Load Key Parameters
		RecKeys = sys_admin_menu.GetRecordKeys() ' Load record keys
		sFilter = sys_admin_menu.GetKeyFilter()
		If sFilter = "" Then
			Call Page_Terminate("pom_sys_admin_menulist.asp") ' Prevent SQL injection, return to list
		End If

		' Set up filter (Sql Where Clause) and get Return Sql
		' Sql constructor in sys_admin_menu class, sys_admin_menuinfo.asp

		sys_admin_menu.CurrentFilter = sFilter

		' Get action
		If Request.Form("a_delete").Count > 0 Then
			sys_admin_menu.CurrentAction = Request.Form("a_delete")
		Else
			sys_admin_menu.CurrentAction = "I"	' Display record
		End If
		Select Case sys_admin_menu.CurrentAction
			Case "D" ' Delete
				sys_admin_menu.SendEmail = True ' Send email on delete success
				If DeleteRows() Then ' Delete rows
					SuccessMessage = Language.Phrase("DeleteSuccess") ' Set up success message
					Call Page_Terminate(sys_admin_menu.ReturnUrl) ' Return to caller
				End If
		End Select
	End Sub
	...
	Function DeleteRows()
		...
		If DeleteRows Then
			...
			Do While Not RsDelete.Eof
				DeleteRows = sys_admin_menu.Row_Deleting(RsDelete)
		...
		If DeleteRows Then
			...
			Do While Not RsDelete.Eof
				...
				sThisKey = sThisKey & RsDelete("sys_admin_menu_id")
		...
		If DeleteRows Then
			...
			Do While Not RsOld.Eof
				Call sys_admin_menu.Row_Deleted(RsOld)
				RsOld.MoveNext
			Loop
		End If
		RsOld.Close
