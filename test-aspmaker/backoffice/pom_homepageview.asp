...
' Define page object
Dim homepage_view
Set homepage_view = New chomepage_view
Set Page = homepage_view

' Page init processing
homepage_view.Page_Init()

' Page main processing
homepage_view.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
homepage_view.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<% If homepage.Export = "" Then %>
<script type="text/javascript">
// Page object
var homepage_view = new ew_Page("homepage_view");
homepage_view.PageID = "view"; // Page ID
var EW_PAGE_ID = homepage_view.PageID; // For backward compatibility
// Form object
var fhomepageview = new ew_Form("fhomepageview");
// Form_CustomValidate event
fhomepageview.Form_CustomValidate = 
...
fhomepageview.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fhomepageview.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% homepage_view.ExportOptions.Render "body", "", "", "", "", "" %>
<% If Not homepage_view.ExportOptions.UseDropDownButton Then %>
</div>
<div class="ewViewOtherOptions">
<% End If %>
<%
	homepage_view.ActionOptions.Render "body", "", "", "", "", ""
	homepage_view.DetailOptions.Render "body", "", "", "", "", ""
%>
</div>
<% End If %>
<% homepage_view.ShowPageHeader() %>
<% homepage_view.ShowMessage %>
<form name="fhomepageview" id="fhomepageview" class="ewForm form-inline" action="<%= ew_CurrentPage %>" method="post">
<input type="hidden" name="t" value="homepage">
<table class="ewGrid"><tr><td>
<table id="tbl_homepageview" class="table table-bordered table-striped">
...
Class chomepage_view
	...
	Private Sub Class_Initialize()
		...
		If Request.QueryString("hp_id").Count > 0 Then
			ew_AddKey RecKey, "hp_id", Request.QueryString("hp_id")
			KeyUrl = KeyUrl & "&amp;hp_id=" & Server.URLEncode(Request.QueryString("hp_id"))
		End If
	...
	Sub Page_Main()
		...
		If IsPageRequest Then ' Validate request
			...
			Else
				sReturnUrl = "pom_homepagelist.asp" ' Return to list
			End If

			' Get action
			homepage.CurrentAction = "I" ' Display form
			Select Case homepage.CurrentAction
				Case "I" ' Get a record to display
					If Not LoadRow() Then ' Load record based on key
						...
						End If
						sReturnUrl = "pom_homepagelist.asp" ' No matching record, return to list
					End If
			End Select
		Else
			sReturnUrl = "pom_homepagelist.asp" ' Not page request, return to list
		End If
		If sReturnUrl <> "" Then Call Page_Terminate(sReturnUrl)

		' Render row
		homepage.RowType = EW_ROWTYPE_VIEW
