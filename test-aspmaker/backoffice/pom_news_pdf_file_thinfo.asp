...
' ASPMaker configuration for Table news_pdf_file_th
Dim news_pdf_file_th

' Define table class
Class cnews_pdf_file_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "news_pdf_id", news_pdf_id)
		Call ew_SetArObj(Fields, "news_id", news_id)
		Call ew_SetArObj(Fields, "news_pdf_file", news_pdf_file)
		Call ew_SetArObj(Fields, "news_pdf_title", news_pdf_title)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "news_pdf_file_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [news_pdf_file_th]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[news_pdf_id] = @news_pdf_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(news_pdf_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@news_pdf_id@", ew_AdjustSql(news_pdf_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_news_pdf_file_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_news_pdf_file_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_news_pdf_file_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_news_pdf_file_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_news_pdf_file_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_news_pdf_file_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_news_pdf_file_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(news_pdf_id.CurrentValue) Then
			sUrl = sUrl & "news_pdf_id=" & news_pdf_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("news_pdf_id") ' news_pdf_id
	...
	Public Sub LoadListRowValues(RsRow)
		news_pdf_id.DbValue = RsRow("news_pdf_id")
		news_id.DbValue = RsRow("news_id")
		news_pdf_file.DbValue = RsRow("news_pdf_file")
		news_pdf_title.DbValue = RsRow("news_pdf_title")
	End Sub
	...
	Sub RenderListRow()
		...
		news_pdf_id.ViewValue = news_pdf_id.CurrentValue
		news_pdf_id.ViewCustomAttributes = ""
		...
		' news_pdf_file
		news_pdf_file.ViewValue = news_pdf_file.CurrentValue
		news_pdf_file.ViewCustomAttributes = ""

		' news_pdf_title
		news_pdf_title.ViewValue = news_pdf_title.CurrentValue
		news_pdf_title.ViewCustomAttributes = ""

		' news_pdf_id
		news_pdf_id.LinkCustomAttributes = ""
		news_pdf_id.HrefValue = ""
		news_pdf_id.TooltipValue = ""
		...
		' news_pdf_file
		news_pdf_file.LinkCustomAttributes = ""
		news_pdf_file.HrefValue = ""
		news_pdf_file.TooltipValue = ""

		' news_pdf_title
		news_pdf_title.LinkCustomAttributes = ""
		news_pdf_title.HrefValue = ""
		news_pdf_title.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [news_pdf_file_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("news_pdf_id", news_pdf_id.ExportValue(Export))
					Call XmlDoc.AddField("news_id", news_id.ExportValue(Export))
					Call XmlDoc.AddField("news_pdf_file", news_pdf_file.ExportValue(Export))
					Call XmlDoc.AddField("news_pdf_title", news_pdf_title.ExportValue(Export))
				Else
					Call XmlDoc.AddField("news_pdf_id", news_pdf_id.ExportValue(Export))
					Call XmlDoc.AddField("news_id", news_id.ExportValue(Export))
					Call XmlDoc.AddField("news_pdf_file", news_pdf_file.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If news_pdf_id.Exportable Then Call Doc.ExportCaption(news_pdf_id)
				If news_id.Exportable Then Call Doc.ExportCaption(news_id)
				If news_pdf_file.Exportable Then Call Doc.ExportCaption(news_pdf_file)
				If news_pdf_title.Exportable Then Call Doc.ExportCaption(news_pdf_title)
			Else
				If news_pdf_id.Exportable Then Call Doc.ExportCaption(news_pdf_id)
				If news_id.Exportable Then Call Doc.ExportCaption(news_id)
				If news_pdf_file.Exportable Then Call Doc.ExportCaption(news_pdf_file)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If news_pdf_id.Exportable Then Call Doc.ExportField(news_pdf_id)
					If news_id.Exportable Then Call Doc.ExportField(news_id)
					If news_pdf_file.Exportable Then Call Doc.ExportField(news_pdf_file)
					If news_pdf_title.Exportable Then Call Doc.ExportField(news_pdf_title)
				Else
					If news_pdf_id.Exportable Then Call Doc.ExportField(news_pdf_id)
					If news_id.Exportable Then Call Doc.ExportField(news_id)
					If news_pdf_file.Exportable Then Call Doc.ExportField(news_pdf_file)
...
'			End If
	...
	' ----------------
	' Field news_pdf_id
	Private m_news_pdf_id

	Public Property Get news_pdf_id()
		If Not IsObject(m_news_pdf_id) Then
			Set m_news_pdf_id = NewFldObj("news_pdf_file_th", "news_pdf_file_th", "x_news_pdf_id", "news_pdf_id", "[news_pdf_id]", "[news_pdf_id]", 3, 0, "[news_pdf_id]", False, False, FALSE, "FORMATTED TEXT")
			m_news_pdf_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set news_pdf_id = m_news_pdf_id
	End Property
	...
	Public Property Get news_id()
		If Not IsObject(m_news_id) Then
			Set m_news_id = NewFldObj("news_pdf_file_th", "news_pdf_file_th", "x_news_id", "news_id", "[news_id]", "[news_id]", 3, 0, "[news_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	' Field news_pdf_file
	Private m_news_pdf_file

	Public Property Get news_pdf_file()
		If Not IsObject(m_news_pdf_file) Then
			Set m_news_pdf_file = NewFldObj("news_pdf_file_th", "news_pdf_file_th", "x_news_pdf_file", "news_pdf_file", "[news_pdf_file]", "[news_pdf_file]", 202, 0, "[news_pdf_file]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set news_pdf_file = m_news_pdf_file
	End Property

	' Field news_pdf_title
	Private m_news_pdf_title

	Public Property Get news_pdf_title()
		If Not IsObject(m_news_pdf_title) Then
			Set m_news_pdf_title = NewFldObj("news_pdf_file_th", "news_pdf_file_th", "x_news_pdf_title", "news_pdf_title", "[news_pdf_title]", "[news_pdf_title]", 202, 0, "[news_pdf_title]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set news_pdf_title = m_news_pdf_title
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_news_pdf_id) Then Set m_news_pdf_id = Nothing
		If IsObject(m_news_id) Then Set m_news_id = Nothing
		If IsObject(m_news_pdf_file) Then Set m_news_pdf_file = Nothing
		If IsObject(m_news_pdf_title) Then Set m_news_pdf_title = Nothing
