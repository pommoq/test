...
' ASPMaker configuration for Table company_th
Dim company_th
...
Class ccompany_th
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "company_name_en", company_name_en)
		Call ew_SetArObj(Fields, "company_name_th", company_name_th)
		Call ew_SetArObj(Fields, "company_create", company_create)
		Call ew_SetArObj(Fields, "company_update", company_update)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "company_th_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [company_th]"
	End Property
	...
	Private Property Get SqlKeyFilter()
		SqlKeyFilter = "[company_id] = @company_id@"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(company_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@company_id@", ew_AdjustSql(company_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_company_thlist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_company_thlist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_company_thview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_company_thview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_company_thedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_company_thadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_company_thdelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(company_id.CurrentValue) Then
			sUrl = sUrl & "company_id=" & company_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("company_id") ' company_id
	...
	Public Sub LoadListRowValues(RsRow)
		...
		company_name_en.DbValue = RsRow("company_name_en")
		company_name_th.DbValue = RsRow("company_name_th")
		company_create.DbValue = RsRow("company_create")
		company_update.DbValue = RsRow("company_update")
	End Sub
	...
	Sub RenderListRow()
		...
		' company_name_en
		company_name_en.ViewValue = company_name_en.CurrentValue
		company_name_en.ViewCustomAttributes = ""

		' company_name_th
		company_name_th.ViewValue = company_name_th.CurrentValue
		company_name_th.ViewCustomAttributes = ""

		' company_create
		company_create.ViewValue = company_create.CurrentValue
		company_create.ViewCustomAttributes = ""

		' company_update
		company_update.ViewValue = company_update.CurrentValue
		company_update.ViewCustomAttributes = ""
		...
		' company_name_en
		company_name_en.LinkCustomAttributes = ""
		company_name_en.HrefValue = ""
		company_name_en.TooltipValue = ""

		' company_name_th
		company_name_th.LinkCustomAttributes = ""
		company_name_th.HrefValue = ""
		company_name_th.TooltipValue = ""

		' company_create
		company_create.LinkCustomAttributes = ""
		company_create.HrefValue = ""
		company_create.TooltipValue = ""

		' company_update
		company_update.LinkCustomAttributes = ""
		company_update.HrefValue = ""
		company_update.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [company_th] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					Call XmlDoc.AddField("company_name_en", company_name_en.ExportValue(Export))
					Call XmlDoc.AddField("company_name_th", company_name_th.ExportValue(Export))
					Call XmlDoc.AddField("company_create", company_create.ExportValue(Export))
					Call XmlDoc.AddField("company_update", company_update.ExportValue(Export))
				Else
					Call XmlDoc.AddField("company_id", company_id.ExportValue(Export))
					Call XmlDoc.AddField("company_name_en", company_name_en.ExportValue(Export))
					Call XmlDoc.AddField("company_name_th", company_name_th.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				...
				If company_name_en.Exportable Then Call Doc.ExportCaption(company_name_en)
				If company_name_th.Exportable Then Call Doc.ExportCaption(company_name_th)
				If company_create.Exportable Then Call Doc.ExportCaption(company_create)
				If company_update.Exportable Then Call Doc.ExportCaption(company_update)
			Else
				If company_id.Exportable Then Call Doc.ExportCaption(company_id)
				If company_name_en.Exportable Then Call Doc.ExportCaption(company_name_en)
				If company_name_th.Exportable Then Call Doc.ExportCaption(company_name_th)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					...
					If company_name_en.Exportable Then Call Doc.ExportField(company_name_en)
					If company_name_th.Exportable Then Call Doc.ExportField(company_name_th)
					If company_create.Exportable Then Call Doc.ExportField(company_create)
					If company_update.Exportable Then Call Doc.ExportField(company_update)
				Else
					If company_id.Exportable Then Call Doc.ExportField(company_id)
					If company_name_en.Exportable Then Call Doc.ExportField(company_name_en)
					If company_name_th.Exportable Then Call Doc.ExportField(company_name_th)
...
'			End If
	...
	Public Property Get company_id()
		If Not IsObject(m_company_id) Then
			Set m_company_id = NewFldObj("company_th", "company_th", "x_company_id", "company_id", "[company_id]", "[company_id]", 3, 0, "[company_id]", False, False, FALSE, "FORMATTED TEXT")
	...
	Private m_company_name_en

	Public Property Get company_name_en()
		If Not IsObject(m_company_name_en) Then
			Set m_company_name_en = NewFldObj("company_th", "company_th", "x_company_name_en", "company_name_en", "[company_name_en]", "[company_name_en]", 202, 0, "[company_name_en]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set company_name_en = m_company_name_en
	End Property

	' Field company_name_th
	Private m_company_name_th

	Public Property Get company_name_th()
		If Not IsObject(m_company_name_th) Then
			Set m_company_name_th = NewFldObj("company_th", "company_th", "x_company_name_th", "company_name_th", "[company_name_th]", "[company_name_th]", 202, 0, "[company_name_th]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set company_name_th = m_company_name_th
	End Property

	' Field company_create
	Private m_company_create

	Public Property Get company_create()
		If Not IsObject(m_company_create) Then
			Set m_company_create = NewFldObj("company_th", "company_th", "x_company_create", "company_create", "[company_create]", "[company_create]", 202, 0, "[company_create]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set company_create = m_company_create
	End Property

	' Field company_update
	Private m_company_update

	Public Property Get company_update()
		If Not IsObject(m_company_update) Then
			Set m_company_update = NewFldObj("company_th", "company_th", "x_company_update", "company_update", "[company_update]", "[company_update]", 202, 0, "[company_update]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set company_update = m_company_update
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		...
		If IsObject(m_company_name_en) Then Set m_company_name_en = Nothing
		If IsObject(m_company_name_th) Then Set m_company_name_th = Nothing
		If IsObject(m_company_create) Then Set m_company_create = Nothing
		If IsObject(m_company_update) Then Set m_company_update = Nothing
