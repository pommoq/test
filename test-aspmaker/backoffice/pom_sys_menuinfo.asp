...
' ASPMaker configuration for Table sys_menu
Dim sys_menu
...
Class csys_menu
	...
	Private Sub Class_Initialize()
		...
		Call ew_SetArObj(Fields, "menu_id", menu_id)
		Call ew_SetArObj(Fields, "menu_name", menu_name)
		Call ew_SetArObj(Fields, "menu_parent_id", menu_parent_id)
		Call ew_SetArObj(Fields, "menu_thai", menu_thai)
		Call ew_SetArObj(Fields, "menu_idname", menu_idname)
		Call ew_SetArObj(Fields, "menu_filename", menu_filename)
		Call ew_SetArObj(Fields, "target", target)
		Call ew_SetArObj(Fields, "OrderList", OrderList)
	End Sub
	...
	Public Property Get HighlightName()
		HighlightName = "sys_menu_Highlight"
	End Property
	...
	Public Property Get SqlSelect() ' Select
		SqlSelect = "SELECT * FROM [sys_menu]"
	End Property
	...
	Public Property Get KeyFilter()
		...
		If Not IsNumeric(menu_id.CurrentValue) Then
			sKeyFilter = "0=1" ' Invalid key
		End If
		sKeyFilter = Replace(sKeyFilter, "@menu_id@", ew_AdjustSql(menu_id.CurrentValue)) ' Replace key value
	...
	Public Property Get ReturnUrl()
		...
		Else
			ReturnUrl = "pom_sys_menulist.asp"
		End If
	End Property

	' List url
	Public Function ListUrl()
		ListUrl = "pom_sys_menulist.asp"
	End Function

	' View url
	Public Function ViewUrl(parm)
		If parm <> "" Then
			ViewUrl = KeyUrl("pom_sys_menuview.asp", UrlParm(parm))
		Else
			ViewUrl = KeyUrl("pom_sys_menuview.asp", UrlParm(EW_TABLE_SHOW_DETAIL & "="))
		End If
	End Function
...
'		If sUrlParm <> "" Then AddUrl = AddUrl & "?" & sUrlParm
	...
	Public Function EditUrl(parm)
		EditUrl = KeyUrl("pom_sys_menuedit.asp", UrlParm(parm))
	End Function
	...
	Public Function CopyUrl(parm)
		CopyUrl = KeyUrl("pom_sys_menuadd.asp", UrlParm(parm))
	End Function
	...
	Public Function DeleteUrl()
		DeleteUrl = KeyUrl("pom_sys_menudelete.asp", UrlParm(""))
	End Function
	...
	Public Function KeyUrl(url, parm)
		...
		If Not IsNull(menu_id.CurrentValue) Then
			sUrl = sUrl & "menu_id=" & menu_id.CurrentValue
		Else
	...
	Public Function GetRecordKeys()
		...
		If Not bHasKey Then
			...
			ElseIf Request.QueryString <> "" Then
				ReDim arKeys(0)
				arKeys(0) = Request.QueryString("menu_id") ' menu_id
	...
	Public Sub LoadListRowValues(RsRow)
		menu_id.DbValue = RsRow("menu_id")
		menu_name.DbValue = RsRow("menu_name")
		menu_parent_id.DbValue = RsRow("menu_parent_id")
		menu_thai.DbValue = RsRow("menu_thai")
		menu_idname.DbValue = RsRow("menu_idname")
		menu_filename.DbValue = RsRow("menu_filename")
		target.DbValue = RsRow("target")
		OrderList.DbValue = RsRow("OrderList")
	End Sub
	...
	Sub RenderListRow()
		...
		menu_id.ViewValue = menu_id.CurrentValue
		menu_id.ViewCustomAttributes = ""

		' menu_name
		menu_name.ViewValue = menu_name.CurrentValue
		menu_name.ViewCustomAttributes = ""

		' menu_parent_id
		menu_parent_id.ViewValue = menu_parent_id.CurrentValue
		menu_parent_id.ViewCustomAttributes = ""

		' menu_thai
		menu_thai.ViewValue = menu_thai.CurrentValue
		menu_thai.ViewCustomAttributes = ""

		' menu_idname
		menu_idname.ViewValue = menu_idname.CurrentValue
		menu_idname.ViewCustomAttributes = ""

		' menu_filename
		menu_filename.ViewValue = menu_filename.CurrentValue
		menu_filename.ViewCustomAttributes = ""

		' target
		target.ViewValue = target.CurrentValue
		target.ViewCustomAttributes = ""

		' OrderList
		OrderList.ViewValue = OrderList.CurrentValue
		OrderList.ViewCustomAttributes = ""

		' menu_id
		menu_id.LinkCustomAttributes = ""
		menu_id.HrefValue = ""
		menu_id.TooltipValue = ""

		' menu_name
		menu_name.LinkCustomAttributes = ""
		menu_name.HrefValue = ""
		menu_name.TooltipValue = ""

		' menu_parent_id
		menu_parent_id.LinkCustomAttributes = ""
		menu_parent_id.HrefValue = ""
		menu_parent_id.TooltipValue = ""

		' menu_thai
		menu_thai.LinkCustomAttributes = ""
		menu_thai.HrefValue = ""
		menu_thai.TooltipValue = ""

		' menu_idname
		menu_idname.LinkCustomAttributes = ""
		menu_idname.HrefValue = ""
		menu_idname.TooltipValue = ""

		' menu_filename
		menu_filename.LinkCustomAttributes = ""
		menu_filename.HrefValue = ""
		menu_filename.TooltipValue = ""

		' target
		target.LinkCustomAttributes = ""
		target.HrefValue = ""
		target.TooltipValue = ""

		' OrderList
		OrderList.LinkCustomAttributes = ""
		OrderList.HrefValue = ""
		OrderList.TooltipValue = ""
	...
	Function DeleteDetailRecords(Rs, Where)
		...
		If IsNull(Rs) Then
			Dim rsfile
			sSql = "SELECT * FROM [sys_menu] WHERE " & sWhereList
		...
		Do While Not Recordset.Eof And RecCnt < StopRec
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					Call XmlDoc.AddField("menu_id", menu_id.ExportValue(Export))
					Call XmlDoc.AddField("menu_name", menu_name.ExportValue(Export))
					Call XmlDoc.AddField("menu_parent_id", menu_parent_id.ExportValue(Export))
					Call XmlDoc.AddField("menu_thai", menu_thai.ExportValue(Export))
					Call XmlDoc.AddField("menu_idname", menu_idname.ExportValue(Export))
					Call XmlDoc.AddField("menu_filename", menu_filename.ExportValue(Export))
					Call XmlDoc.AddField("target", target.ExportValue(Export))
					Call XmlDoc.AddField("OrderList", OrderList.ExportValue(Export))
				Else
					Call XmlDoc.AddField("menu_id", menu_id.ExportValue(Export))
					Call XmlDoc.AddField("menu_name", menu_name.ExportValue(Export))
					Call XmlDoc.AddField("menu_parent_id", menu_parent_id.ExportValue(Export))
	...
	Public Sub ExportDocument(Doc, Recordset, StartRec, StopRec, ExportPageType)
		...
		If Doc.Horizontal Then ' Horizontal format, write header
			...
			If ExportPageType = "view" Then
				If menu_id.Exportable Then Call Doc.ExportCaption(menu_id)
				If menu_name.Exportable Then Call Doc.ExportCaption(menu_name)
				If menu_parent_id.Exportable Then Call Doc.ExportCaption(menu_parent_id)
				If menu_thai.Exportable Then Call Doc.ExportCaption(menu_thai)
				If menu_idname.Exportable Then Call Doc.ExportCaption(menu_idname)
				If menu_filename.Exportable Then Call Doc.ExportCaption(menu_filename)
				If target.Exportable Then Call Doc.ExportCaption(target)
				If OrderList.Exportable Then Call Doc.ExportCaption(OrderList)
			Else
				If menu_id.Exportable Then Call Doc.ExportCaption(menu_id)
				If menu_name.Exportable Then Call Doc.ExportCaption(menu_name)
				If menu_parent_id.Exportable Then Call Doc.ExportCaption(menu_parent_id)
		...
		Do While Not Recordset.Eof And CLng(RecCnt) < CLng(StopRec)
			...
			If CLng(RecCnt) >= CLng(StartRec) Then
				...
				If ExportPageType = "view" Then
					If menu_id.Exportable Then Call Doc.ExportField(menu_id)
					If menu_name.Exportable Then Call Doc.ExportField(menu_name)
					If menu_parent_id.Exportable Then Call Doc.ExportField(menu_parent_id)
					If menu_thai.Exportable Then Call Doc.ExportField(menu_thai)
					If menu_idname.Exportable Then Call Doc.ExportField(menu_idname)
					If menu_filename.Exportable Then Call Doc.ExportField(menu_filename)
					If target.Exportable Then Call Doc.ExportField(target)
					If OrderList.Exportable Then Call Doc.ExportField(OrderList)
				Else
					If menu_id.Exportable Then Call Doc.ExportField(menu_id)
					If menu_name.Exportable Then Call Doc.ExportField(menu_name)
					If menu_parent_id.Exportable Then Call Doc.ExportField(menu_parent_id)
...
'			End If
	...
	Public Property Get menu_id()
		If Not IsObject(m_menu_id) Then
			Set m_menu_id = NewFldObj("sys_menu", "sys_menu", "x_menu_id", "menu_id", "[menu_id]", "[menu_id]", 3, 0, "[menu_id]", False, False, FALSE, "FORMATTED TEXT")
			m_menu_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set menu_id = m_menu_id
	End Property

	' Field menu_name
	Private m_menu_name

	Public Property Get menu_name()
		If Not IsObject(m_menu_name) Then
			Set m_menu_name = NewFldObj("sys_menu", "sys_menu", "x_menu_name", "menu_name", "[menu_name]", "[menu_name]", 202, 0, "[menu_name]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set menu_name = m_menu_name
	End Property

	' Field menu_parent_id
	Private m_menu_parent_id

	Public Property Get menu_parent_id()
		If Not IsObject(m_menu_parent_id) Then
			Set m_menu_parent_id = NewFldObj("sys_menu", "sys_menu", "x_menu_parent_id", "menu_parent_id", "[menu_parent_id]", "[menu_parent_id]", 3, 0, "[menu_parent_id]", False, False, FALSE, "FORMATTED TEXT")
			m_menu_parent_id.FldDefaultErrMsg = Language.Phrase("IncorrectInteger")
		End If
		Set menu_parent_id = m_menu_parent_id
	End Property

	' Field menu_thai
	Private m_menu_thai

	Public Property Get menu_thai()
		If Not IsObject(m_menu_thai) Then
			Set m_menu_thai = NewFldObj("sys_menu", "sys_menu", "x_menu_thai", "menu_thai", "[menu_thai]", "[menu_thai]", 202, 0, "[menu_thai]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set menu_thai = m_menu_thai
	End Property

	' Field menu_idname
	Private m_menu_idname

	Public Property Get menu_idname()
		If Not IsObject(m_menu_idname) Then
			Set m_menu_idname = NewFldObj("sys_menu", "sys_menu", "x_menu_idname", "menu_idname", "[menu_idname]", "[menu_idname]", 202, 0, "[menu_idname]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set menu_idname = m_menu_idname
	End Property

	' Field menu_filename
	Private m_menu_filename

	Public Property Get menu_filename()
		If Not IsObject(m_menu_filename) Then
			Set m_menu_filename = NewFldObj("sys_menu", "sys_menu", "x_menu_filename", "menu_filename", "[menu_filename]", "[menu_filename]", 202, 0, "[menu_filename]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set menu_filename = m_menu_filename
	End Property
	...
	Public Property Get target()
		If Not IsObject(m_target) Then
			Set m_target = NewFldObj("sys_menu", "sys_menu", "x_target", "target", "[target]", "[target]", 202, 0, "[target]", False, False, FALSE, "FORMATTED TEXT")
		End If
		Set target = m_target
	End Property
	...
	Public Property Get OrderList()
		If Not IsObject(m_OrderList) Then
			Set m_OrderList = NewFldObj("sys_menu", "sys_menu", "x_OrderList", "OrderList", "[OrderList]", "[OrderList]", 5, 0, "[OrderList]", False, False, FALSE, "FORMATTED TEXT")
			m_OrderList.FldDefaultErrMsg = Language.Phrase("IncorrectFloat")
		End If
		Set OrderList = m_OrderList
	End Property
	Dim Fields ' Fields
	...
	Private Sub Class_Terminate
		If IsObject(m_menu_id) Then Set m_menu_id = Nothing
		If IsObject(m_menu_name) Then Set m_menu_name = Nothing
		If IsObject(m_menu_parent_id) Then Set m_menu_parent_id = Nothing
		If IsObject(m_menu_thai) Then Set m_menu_thai = Nothing
		If IsObject(m_menu_idname) Then Set m_menu_idname = Nothing
		If IsObject(m_menu_filename) Then Set m_menu_filename = Nothing
		If IsObject(m_target) Then Set m_target = Nothing
		If IsObject(m_OrderList) Then Set m_OrderList = Nothing
