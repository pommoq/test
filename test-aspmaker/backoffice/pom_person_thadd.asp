...
' Define page object
Dim person_th_add
Set person_th_add = New cperson_th_add
Set Page = person_th_add

' Page init processing
person_th_add.Page_Init()

' Page main processing
person_th_add.Page_Main()

' Global Page Rendering event (in userfn*.asp)
Page_Rendering()

' Page Rendering event
person_th_add.Page_Render()
%>
<!--#include file="pom_header.asp"-->
<script type="text/javascript">
// Page object
var person_th_add = new ew_Page("person_th_add");
person_th_add.PageID = "add"; // Page ID
var EW_PAGE_ID = person_th_add.PageID; // For backward compatibility
// Form object
var fperson_thadd = new ew_Form("fperson_thadd");
// Validate form
fperson_thadd.Validate = function() {
...
fperson_thadd.Form_CustomValidate = 
...
fperson_thadd.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fperson_thadd.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% Breadcrumb.Render() %>
<% End If %>
<% person_th_add.ShowPageHeader() %>
<% person_th_add.ShowMessage %>
<form name="fperson_thadd" id="fperson_thadd" class="ewForm form-inline" action="<%= ew_CurrentPage() %>" method="post">
<input type="hidden" name="t" value="person_th">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_person_thadd" class="table table-bordered table-striped">
...
<input type="text" data-field="x_per_id" name="x_per_id" id="x_per_id" size="30" placeholder="<%= person_th.per_id.PlaceHolder %>" value="<%= person_th.per_id.EditValue %>"<%= person_th.per_id.EditAttributes %>>
</span>
...
Class cperson_th_add
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			person_th.CurrentAction = ObjForm.GetValue("a_add") ' Get form action
		...
		Else
			...
			If Request.QueryString("per_id").Count > 0 Then
				...
				Call person_th.SetKey("per_id", person_th.per_id.CurrentValue) ' Set up key
			Else
				Call person_th.SetKey("per_id", "") ' Clear key
				CopyRecord = False
			End If
			If CopyRecord Then
				person_th.CurrentAction = "C" ' Copy Record
			Else
				person_th.CurrentAction = "I" ' Display Blank Record
		...
		If ObjForm.GetValue("a_add")&"" <> "" Then
			If Not ValidateForm() Then
				person_th.CurrentAction = "I" ' Form error, reset action
		...
		Select Case person_th.CurrentAction
			...
			Case "A" ' Add new record
				person_th.SendEmail = True ' Send email on add success
				If AddRow(OldRecordset) Then ' Add successful
					SuccessMessage = Language.Phrase("AddSuccess") ' Set up success message
					Dim sReturnUrl
					sReturnUrl = person_th.ReturnUrl
					If ew_GetPageName(sReturnUrl) = "pom_person_thview.asp" Then sReturnUrl = person_th.ViewUrl("") ' View paging, return to view page with keyurl directly
		...
		person_th.RowType = EW_ROWTYPE_ADD ' Render add type
	...
	Function LoadDefaultValues()
		person_th.per_id.CurrentValue = Null
		person_th.per_id.OldValue = person_th.per_id.CurrentValue
		person_th.dept_id.CurrentValue = Null
		person_th.dept_id.OldValue = person_th.dept_id.CurrentValue
		person_th.office_id.CurrentValue = Null
		person_th.office_id.OldValue = person_th.office_id.CurrentValue
		person_th.per_img.CurrentValue = Null
		person_th.per_img.OldValue = person_th.per_img.CurrentValue
		person_th.per_en_name.CurrentValue = Null
		person_th.per_en_name.OldValue = person_th.per_en_name.CurrentValue
		person_th.per_th_name.CurrentValue = Null
		person_th.per_th_name.OldValue = person_th.per_th_name.CurrentValue
		person_th.per_position.CurrentValue = Null
		person_th.per_position.OldValue = person_th.per_position.CurrentValue
		person_th.per_mobile.CurrentValue = Null
		person_th.per_mobile.OldValue = person_th.per_mobile.CurrentValue
		person_th.per_tel.CurrentValue = Null
		person_th.per_tel.OldValue = person_th.per_tel.CurrentValue
		person_th.per_fax.CurrentValue = Null
		person_th.per_fax.OldValue = person_th.per_fax.CurrentValue
		person_th.per_email.CurrentValue = Null
		person_th.per_email.OldValue = person_th.per_email.CurrentValue
		person_th.per_address.CurrentValue = Null
		person_th.per_address.OldValue = person_th.per_address.CurrentValue
		person_th.per_show.CurrentValue = Null
		person_th.per_show.OldValue = person_th.per_show.CurrentValue
		person_th.per_create.CurrentValue = Null
		person_th.per_create.OldValue = person_th.per_create.CurrentValue
		person_th.per_update.CurrentValue = Null
		person_th.per_update.OldValue = person_th.per_update.CurrentValue
		person_th.per_sort.CurrentValue = Null
		person_th.per_sort.OldValue = person_th.per_sort.CurrentValue
		person_th.per_department.CurrentValue = Null
		person_th.per_department.OldValue = person_th.per_department.CurrentValue
	End Function
	...
	Sub RenderRow()
		...
		ElseIf person_th.RowType = EW_ROWTYPE_ADD Then ' Add row

			' per_id
			person_th.per_id.EditCustomAttributes = ""
			person_th.per_id.EditValue = ew_HtmlEncode(person_th.per_id.CurrentValue)
			person_th.per_id.PlaceHolder = ew_HtmlEncode(ew_RemoveHtml(person_th.per_id.FldCaption))
	...
	Function AddRow(RsOld)
		...
		If person_th.per_id.CurrentValue <> "" Then ' Check field with unique index
			sFilter = "([per_id] = " & ew_AdjustSql(person_th.per_id.CurrentValue) & ")"
			Set RsChk = person_th.LoadRs(sFilter)
			If Not (RsChk Is Nothing) Then
				sIdxErrMsg = Replace(Language.Phrase("DupIndex"), "%f", person_th.per_id.FldCaption)
				sIdxErrMsg = Replace(sIdxErrMsg, "%v", person_th.per_id.CurrentValue)
		...
		' Field per_id
		Call person_th.per_id.SetDbValue(Rs, person_th.per_id.CurrentValue, Null, False)

		' Field dept_id
		Call person_th.dept_id.SetDbValue(Rs, person_th.dept_id.CurrentValue, Null, False)

		' Field office_id
		Call person_th.office_id.SetDbValue(Rs, person_th.office_id.CurrentValue, Null, False)

		' Field per_img
		Call person_th.per_img.SetDbValue(Rs, person_th.per_img.CurrentValue, Null, False)

		' Field per_en_name
		Call person_th.per_en_name.SetDbValue(Rs, person_th.per_en_name.CurrentValue, Null, False)

		' Field per_th_name
		Call person_th.per_th_name.SetDbValue(Rs, person_th.per_th_name.CurrentValue, Null, False)

		' Field per_position
		Call person_th.per_position.SetDbValue(Rs, person_th.per_position.CurrentValue, Null, False)

		' Field per_mobile
		Call person_th.per_mobile.SetDbValue(Rs, person_th.per_mobile.CurrentValue, Null, False)

		' Field per_tel
		Call person_th.per_tel.SetDbValue(Rs, person_th.per_tel.CurrentValue, Null, False)

		' Field per_fax
		Call person_th.per_fax.SetDbValue(Rs, person_th.per_fax.CurrentValue, Null, False)

		' Field per_email
		Call person_th.per_email.SetDbValue(Rs, person_th.per_email.CurrentValue, Null, False)

		' Field per_address
		Call person_th.per_address.SetDbValue(Rs, person_th.per_address.CurrentValue, Null, False)

		' Field per_show
		Call person_th.per_show.SetDbValue(Rs, person_th.per_show.CurrentValue, Null, False)

		' Field per_create
		Call person_th.per_create.SetDbValue(Rs, person_th.per_create.CurrentValue, Null, False)

		' Field per_update
		Call person_th.per_update.SetDbValue(Rs, person_th.per_update.CurrentValue, Null, False)

		' Field per_sort
		Call person_th.per_sort.SetDbValue(Rs, person_th.per_sort.CurrentValue, Null, False)

		' Field per_department
		Call person_th.per_department.SetDbValue(Rs, person_th.per_department.CurrentValue, Null, False)
		...
		bInsertRow = person_th.Row_Inserting(RsOld, Rs)

		' Check if key value entered
		If bInsertRow And person_th.ValidateKey And person_th.per_id.CurrentValue = "" And person_th.per_id.SessionValue = "" Then
		...
		Dim sKeyErrMsg
		If bInsertRow And person_th.ValidateKey Then
			sFilter = person_th.KeyFilter
			Set RsChk = person_th.LoadRs(sFilter)
			If Not (RsChk Is Nothing) Then
		...
		If AddRow Then
			...
			Call person_th.Row_Inserted(RsOld, RsNew)
		End If
	...
	Sub SetupBreadcrumb()
		...
		PageId = ew_IIf(person_th.CurrentAction = "C", "Copy", "Add")
