...
' Define page object
Dim banner_edit
Set banner_edit = New cbanner_edit
Set Page = banner_edit
...
// Form object
var fbanneredit = new ew_Form("fbanneredit");
// Validate form
...
fbanneredit.Form_CustomValidate = 
...
fbanneredit.ValidateRequired = true; // Use JavaScript validation
<% Else %>
fbanneredit.ValidateRequired = false; // No JavaScript validation
<% End If %>
...
<% Breadcrumb.Render() %>
<% End If %>
<% banner_edit.ShowPageHeader() %>
<% banner_edit.ShowMessage %>
<form name="fbanneredit" id="fbanneredit" class="ewForm form-inline" action="<%= ew_CurrentPage %>" method="post">
<input type="hidden" name="a_table" id="a_table" value="banner">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_banneredit" class="table table-bordered table-striped">
...
<%= banner.banner_id.EditValue %>
</span>
</span>
<input type="hidden" data-field="x_banner_id" name="x_banner_id" id="x_banner_id" value="<%= Server.HTMLEncode(banner.banner_id.CurrentValue&"") %>">
...
Class cbanner_edit
	...
	Sub Page_Main()
		...
		If ObjForm.GetValue("a_edit")&"" <> "" Then
			banner.CurrentAction = ObjForm.GetValue("a_edit") ' Get action code
		...
		' Check if valid key
		If banner.banner_id.CurrentValue = "" Then Call Page_Terminate("pom_bannerlist.asp") ' Invalid key, return to list
	...
	Sub RenderRow()
		...
		ElseIf banner.RowType = EW_ROWTYPE_EDIT Then ' Edit row

			' banner_id
			banner.banner_id.EditCustomAttributes = ""
			banner.banner_id.EditValue = banner.banner_id.CurrentValue
			banner.banner_id.ViewCustomAttributes = ""
	...
	Function EditRow()
		...
		Else
			...
			Call banner.banner_img.SetDbValue(Rs, banner.banner_img.CurrentValue, Null, banner.banner_img.ReadOnly)

			' Field banner_link
			Call banner.banner_link.SetDbValue(Rs, banner.banner_link.CurrentValue, Null, banner.banner_link.ReadOnly)

			' Field banner_sort
			Call banner.banner_sort.SetDbValue(Rs, banner.banner_sort.CurrentValue, Null, banner.banner_sort.ReadOnly)

			' Field start_date
			Call banner.start_date.SetDbValue(Rs, banner.start_date.CurrentValue, Null, banner.start_date.ReadOnly)

			' Field end_date
			Call banner.end_date.SetDbValue(Rs, banner.end_date.CurrentValue, Null, banner.end_date.ReadOnly)
			...
			bUpdateRow = banner.Row_Updating(RsOld, Rs)
			If bUpdateRow Then
		...
		If EditRow Then
			Call banner.Row_Updated(RsOld, RsNew)
		End If
		Rs.Close
		Set Rs = Nothing
