...
function CP_tmpReturnFunction(y,m,d) { 
	...
	if (window.CP_targetInput!=null) {
		...
		if (y==0) {window.CP_targetInput.value = ''; return ;}
...
function CP_setThaiYear(diffYear) {
	this.thaiYear = diffYear;
	}
	...
	if (this.displayType=="date" || this.displayType=="week-end") {
		...
		if (this.isShowNavigationDropdowns) {
			...
			for( var yearCounter=year-this.yearSelectStartOffset; yearCounter<=year+this.yearSelectStartOffset; yearCounter++ ) {
				...
				result += '<option value="'+yearCounter+'" '+selected+'>'+(yearCounter + this.thaiYear)+'</option>';
				}
			result += '</select></TD>';
			}
		else {
		...
		if (disabled || this.disabledWeekDays[current_weekday+1]) {
			...
			result += '		<A CLASS="'+this.cssPrefix+'cpTodayText" HREF="javascript:'+windowref+this.returnFunction+'(0,0,0);'+windowref+'CP_hideCalendar(\''+this.index+'\');">'+this.clearText+'</A>\n';
		result += '		<BR>\n';
